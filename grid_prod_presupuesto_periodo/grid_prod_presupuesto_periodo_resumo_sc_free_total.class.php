...
if($_SESSION['sc_session'][$this->Ini->sc_page]['grid_prod_presupuesto_periodo']['proc_pdf'] || $_SESSION['sc_session'][$this->Ini->sc_page]['grid_prod_presupuesto_periodo']['proc_pdf_vert']){ 
   ...
   function totaliza()
   {
      foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_prod_presupuesto_periodo']['tot_geral'] as $ind => $val)
      {
          if ($ind > 0)
          {
...
{ 
   ...
   //---- 
   function adiciona_registro($sum_monto_uf, $sum_iva, $sum_reajuste, $sum_retencion, $sum_avance_informado)
   {
      //----- Total
      if (empty($this->array_total_geral))
      {
         $this->array_total_geral[0] = 1;
         $this->array_total_geral[1] = $sum_monto_uf;
         $this->array_total_geral[2] = $sum_iva;
         $this->array_total_geral[3] = $sum_reajuste;
         $this->array_total_geral[4] = $sum_retencion;
         $this->array_total_geral[5] = $sum_avance_informado;
      }
      else
      {
         $this->array_total_geral[0]++;
         $this->array_total_geral[1] = bcadd($this->array_total_geral[1], $sum_monto_uf, 10);
         $this->array_total_geral[2] = bcadd($this->array_total_geral[2], $sum_iva, 10);
         ...
         $this->array_total_geral[5] = bcadd($this->array_total_geral[5], $sum_avance_informado, 10);
      }
   }
...
//
   function nm_acumula_resumo($nm_tipo="resumo")
   {
     ...
     { 
         $nmgp_select = "SELECT id_partida, fecha_desde, fecha_hasta, dias_desde, dias_hasta, dias_periodo, fecha_desde_factura, fecha_hasta_factura, monto_uf, produccion_neto, reajuste, retencion, multa, avance_informado, avance, avance_periodo, avance_periodo_siguiente, facturado, factura, id, fecha, id_proyecto, observaciones, produccion_total_final from " . $this->Ini->nm_tabela; 
     } 
     else 
     { 
         $nmgp_select = "SELECT id_partida, fecha_desde, fecha_hasta, dias_desde, dias_hasta, dias_periodo, fecha_desde_factura, fecha_hasta_factura, monto_uf, produccion_neto, reajuste, retencion, multa, avance_informado, avance, avance_periodo, avance_periodo_siguiente, facturado, factura, id, fecha, id_proyecto, observaciones, produccion_total_final from " . $this->Ini->nm_tabela; 
...
// 
   function nm_acum_res_unit($rs_res, $nm_tipo="resumo")
   {
            ...
            $this->id = $rs_res->fields[19] ;  
            $this->fecha = $rs_res->fields[20] ;  
