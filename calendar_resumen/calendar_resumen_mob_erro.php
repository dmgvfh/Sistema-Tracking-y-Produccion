...
$old_error_handler = set_error_handler("calendar_resumen_mob_scriptcase_error_handler");

function calendar_resumen_mob_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe="", $err_usr=false)
{
    ...
    if (isset($_SESSION['scriptcase']['calendar_resumen_mob']['contr_erro']) && $_SESSION['scriptcase']['calendar_resumen_mob']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_calendar_resumen_mob->contr_calendar_resumen_mob->Apl_com_erro = true;
            if ($inicial_calendar_resumen_mob->contr_calendar_resumen_mob->NM_ajax_flag)
            {
                $inicial_calendar_resumen_mob->contr_calendar_resumen_mob->NM_ajax_info['errList']['geral_calendar_resumen_mob'][] = $msg;
                return;
            }
            calendar_resumen_mob_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function calendar_resumen_mob_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['calendar_resumen_mob'])  && '' != $_SESSION['scriptcase']['error_icon']['calendar_resumen_mob'])  ? $_SESSION['scriptcase']['error_icon']['calendar_resumen_mob']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['calendar_resumen_mob']) && '' != $_SESSION['scriptcase']['error_close']['calendar_resumen_mob']) ? $_SESSION['scriptcase']['error_close']['calendar_resumen_mob'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['calendar_resumen_mob']) && '' != $_SESSION['scriptcase']['error_span_title']['calendar_resumen_mob']) ? $_SESSION['scriptcase']['error_span_title']['calendar_resumen_mob'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['calendar_resumen_mob']) && '' != $_SESSION['scriptcase']['error_icon_title']['calendar_resumen_mob']) ? $_SESSION['scriptcase']['error_icon_title']['calendar_resumen_mob'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['calendar_resumen']['error_buffer'] .= ob_get_flush();
}
