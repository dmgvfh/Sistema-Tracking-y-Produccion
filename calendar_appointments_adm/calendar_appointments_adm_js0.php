<form name="F2" method=post 
               action="./" 
               target="_self"> 
<input type="hidden" name="appointments_id" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['appointments_id']); ?>">
...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['calendar_appointments_adm']['parms'])) {echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['calendar_appointments_adm']['parms']);} ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="calendar_appointments_adm"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
?> 
var var_btn_btn_customer_var_customer_id = "<?php echo $this->form_encode_input($this->nmgp_dados_form['customers_id']); ?>";
function sc_btn_btn_customer()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_btn_customer(); }, 500);
      return;
    }
    sc_btn_btn_customer_ok();
}
function sc_btn_btn_customer_cancel()
{
}
function sc_btn_btn_customer_ok()
{
  nm_gp_submit('<?php echo $this->Ini->link_grid_customers_address_cons; ?>', '<?php echo $this->nm_location; ?>', '<?php echo "var_customer_id*scin' + var_btn_btn_customer_var_customer_id + '*scoutNMSC_cab*scinN*scout"; ?>', 'inicio', '_blank', '0', '0', 'grid_customers_address');
}
var var_btn_btn_service_details_var_appointment_id = "<?php echo $this->form_encode_input($this->nmgp_dados_form['appointments_id']); ?>";
var var_btn_btn_service_details_var_cur_status = "<?php echo $this->form_encode_input($this->nmgp_dados_form['current_status_id']); ?>";
function sc_btn_btn_service_details()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_btn_service_details(); }, 500);
      return;
    }
    sc_btn_btn_service_details_ok();
}
function sc_btn_btn_service_details_cancel()
{
}
function sc_btn_btn_service_details_ok()
{
  nm_gp_submit('<?php echo $this->Ini->link_tabs_appointments_aba; ?>', '<?php echo $this->nm_location; ?>', '<?php echo "var_appointment_id*scin' + var_btn_btn_service_details_var_appointment_id + '*scoutvar_cur_status*scin' + var_btn_btn_service_details_var_cur_status + '*scout"; ?>', 'inicio', '_blank', '0', '0', 'tabs_appointments');
}
 NM_tp_critica(1);
...
{ 
   var sob_iframe = '';
<?php 
if ($_SESSION['sc_session'][$this->Ini->sc_page]['calendar_appointments_adm']['embutida_form']) {
...
if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['calendar_appointments_adm']['sc_modal']) && $_SESSION['sc_session'][$this->Ini->sc_page]['calendar_appointments_adm']['sc_modal']) {
    echo "   sob_iframe += 'parent.';\r\n";
}
?>
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['calendar_appointments_adm']['where_detal']))
  {
?>  
...
function sc_inline_form(seqRow, keyParams, width, height)
{
  ...
  nm_gp_submit('<?php echo $this->Ini->link_calendar_appointments_adm_inline ?>', '<?php echo $this->nm_location ?>', 'NM_btn_insert*scinN*scoutNM_btn_update*scinS*scoutNM_btn_delete*scinN*scoutNM_btn_navega*scinN*scoutNMSC_modal*scinok*scoutsc_redir_atualiz*scinok*scoutsc_inline_call*scinY*scoutsc_seq_row*scin' + seqRow + '*scout' + callParams, '', 'modal', height, width);
}

function sc_inline_form_add(width, height)
{
  nm_gp_submit('<?php echo $this->Ini->link_calendar_appointments_adm_inline ?>', '<?php echo $this->nm_location ?>', 'NM_btn_insert*scinY*scoutNM_btn_update*scinN*scoutNM_btn_delete*scinN*scoutNM_btn_navega*scinN*scoutNMSC_modal*scinok*scoutsc_redir_atualiz*scinok*scoutsc_inline_call*scinY*scoutnmgp_opcao*scinnovo*scout', '', 'modal', height, width);
}
...
function sc_appointment_recurrent_onchange()
{
...
{
   nm_field_disabled("current_status_id=disabled", "U");
