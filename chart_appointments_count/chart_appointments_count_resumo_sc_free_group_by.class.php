...
class chart_appointments_count_resumo
{
   var $Db;
   var $Erro;
   var $Ini;
   ...
   var $array_total_appointment_start_date;
   var $array_total_current_status_id;
   ...
   function __construct($tipo = "")
   {
      ...
      $this->array_total_appointment_start_date = array();
      $this->array_total_current_status_id      = array();
   ...
   function initializeButtons()
   {
      ...
      $this->nmgp_botoes['chart_exit'] = isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['dashboard_info']['under_dashboard']) && $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['dashboard_info']['under_dashboard'] ? "off" : "on";

      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['dashboard_info']['under_dashboard']) && $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['dashboard_info']['under_dashboard'] && !$_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['dashboard_info']['maximized']) {
          $tmpDashboardApp = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['dashboard_info']['dashboard_app'];
          if (isset($_SESSION['scriptcase']['dashboard_toolbar'][$tmpDashboardApp]['chart_appointments_count'])) {
              $tmpDashboardButtons = $_SESSION['scriptcase']['dashboard_toolbar'][$tmpDashboardApp]['chart_appointments_count'];
   ...
   if ($this->Ini->Embutida_iframe) {
       foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['sub_cons_iframe_btns'] as $BTN => $BTN_opc) {
           $this->nmgp_botoes[$BTN] = $BTN_opc;
       }
   }
      if (isset($_SESSION['scriptcase']['sc_apl_conf']['chart_appointments_count']['btn_display']) && !empty($_SESSION['scriptcase']['sc_apl_conf']['chart_appointments_count']['btn_display']))
      {
          foreach ($_SESSION['scriptcase']['sc_apl_conf']['chart_appointments_count']['btn_display'] as $NM_cada_btn => $NM_cada_opc)
          {
   ...
   { 
	...
	function generateChartImages() {
		...
		$chartList  = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['pivot_charts'];
		$chartFiles = array();

		$_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['phantomjs_export_process'] = true;
		$_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['export_progress_step']     = 'image';
		...
		foreach ($chartList as $chartKey => $chartData) {
			...
			$_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['export_progress_total']++;
		}
		...
		foreach ($chartList as $chartKey => $chartData) {
			...
			$_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['phantomjs_export_file'] = '';
			$_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['export_progress_count']++;

			$this->writeProgressFile();

			$this->Graf->generateChartImage($chartKey);

			if ('' != $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['phantomjs_export_file']) {
				$chartFiles[] = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['phantomjs_export_file'];
			}
		}

		$_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['phantomjs_export_process'] = false;
	...
	function zipChartImages($password = '') {
		...
		$_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['export_progress_step'] = 'zip';

		$this->writeProgressFile();

		$zipFile = $this->zipFileList($chartImages, $password);

		$_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['export_progress_step'] = 'end';
	...
	function writeProgressFile() {
		if ('image' == $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['export_progress_step']) {
			$progress = floor($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['export_progress_count'] * 100 / ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['export_progress_total'] + 1));
			$content  = $this->Ini->Nm_lang['lang_pdff_pcht'] . ": {$_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['export_progress_count']}/{$_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['export_progress_total']}###$progress";

			$content  = $_SESSION['scriptcase']['charset'] != 'UTF-8' ? sc_convert_encoding($content, 'UTF-8', $_SESSION['scriptcase']['charset']) : $content;
		}
		elseif ('zip' == $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['export_progress_step']) {
			$progress = floor($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['export_progress_count'] * 100 / ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['export_progress_total'] + 1));
			$content  = $this->Ini->Nm_lang['lang_chrt_zip_img'] . "###$progress";

			$content  = $_SESSION['scriptcase']['charset'] != 'UTF-8' ? sc_convert_encoding($content, 'UTF-8', $_SESSION['scriptcase']['charset']) : $content;
		}
		elseif ('end' == $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['export_progress_step']) {
			$content = "ok###100";
		}
		else {
			$content = "init###0";
		...
		$f = @fopen("{$this->Ini->root}{$_SESSION['scriptcase']['chart_appointments_count']['glo_nm_path_imag_temp']}/{$_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['export_progress_file']}", 'w');
		fwrite($f, $content);
		fclose($f);
	}
   ...
   function monta_resumo($b_export = false)
   {
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['orig_pesq'] = "resumo";
      $this->sc_where_orig   = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['where_orig'];
      $this->sc_where_atual  = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['where_pesq'];
      $this->sc_where_filtro = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['where_pesq_filtro'];
     if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['campos_busca']) && !empty($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['campos_busca']))
     { 
         $Busca_temp = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['campos_busca'];
       ...
       $this->appointment_start_date_2 = $appointment_start_date_2; 
         ...
         {
             ob_start();
             $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['chart_combination_type'] = $_POST['fc_chart_type'];
         }

       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['pivot_y_axys']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['pivot_y_axys']))
       {
           ...
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['pivot_x_axys']             = array();
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['pivot_y_axys']             = array();
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['summarizing_fields_order'] = array();
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['comb_field_display_type']  = array();
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['chart_display_values']     = array();

           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['chart_combination_type']        = 'bar3d';
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['chart_combination_order_rule']  = 'asc';
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['chart_combination_order_field'] = 'label';
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['chart_combination_order_rule_groupby']["appointment_start_date"]  = 'asc';
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['chart_combination_order_field_groupby']["appointment_start_date"] = 'label';
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['chart_combination_order_rule_groupby']["current_status_id"]  = 'asc';
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['chart_combination_order_field_groupby']["current_status_id"] = 'label';

           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['pivot_y_axys'][0] = 0;
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['pivot_y_axys'][1] = 1;
           ...
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['comb_field_display_type'][2] = '';

           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['chart_display_values'][2] = null;


           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['summarizing_fields_order']['sc_free_group_by'][] = 2;
       }
      ...
      {
          unset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['where_resumo']);
      }
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['res_hrz']))
      { 
          $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['res_hrz'] = $this->NM_totaliz_hrz;
      } 
      $this->NM_totaliz_hrz = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['res_hrz'];
      if (isset($_SESSION['scriptcase']['sc_aba_iframe']) && !$this->NM_export)
      {
          foreach ($_SESSION['scriptcase']['sc_aba_iframe'] as $aba => $apls_aba)
          {
              if (in_array("chart_appointments_count", $apls_aba))
              {
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['iframe_menu'] && !$this->NM_export && (!isset($_SESSION['scriptcase']['menu_mobile']) || empty($_SESSION['scriptcase']['menu_mobile'])))
      {
          $this->aba_iframe = true;
      }
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['array_graf_pdf'] = array();
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['opcao'] != "pdf")
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['where_resumo'] = "";
      }
      $this->inicializa_vars();
      $Gb_geral = "quebra_geral_" . $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['SC_Ind_Groupby'];
      $this->Total->$Gb_geral(false, $this->NM_export);
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['contr_array_resumo'] == "OK" && $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['contr_total_geral'] == "OK")
      {
          foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['SC_Gb_Free_cmp'] as $cmp_gb => $resto)
          {
              $Arr_tot_name = "array_total_" . $cmp_gb;
              $this->$Arr_tot_name = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['arr_total'][$cmp_gb];
          }
      }
      else
      {
      ...
      $this->array_total_geral[0] = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['tot_geral'][2];
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['tot_geral'][1]) || $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['tot_geral'][1] == 0)
      {
      ...
      $this->resumo_final();
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['contr_label_graf'] = array();
      if (isset($this->nmgp_label_quebras) && !empty($this->nmgp_label_quebras))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['contr_label_graf'] = $this->nmgp_label_quebras;
      }
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['arr_total']['appointment_start_date'] = $this->array_total_appointment_start_date;
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['arr_total']['current_status_id'] = $this->array_total_current_status_id;
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['contr_array_resumo'] = "OK";
   }
   ...
   function completeMatrix()
   {
       ...
       //-----
       if ($this->NM_export && isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['json_use_label']) && !$_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['json_use_label'])
       {
          $Lab_appointment_start_date = "appointment_start_date";
          $Lab_current_status_id = "current_status_id";
       }
       else
       {
       $Lab_appointment_start_date = "" . $this->Ini->Nm_lang['lang_appointments_fld_appointment_start_date'] . "";
       $Lab_current_status_id = "" . $this->Ini->Nm_lang['lang_appointments_fld_current_status_id'] . "";
       }
       $prep_field = array();
       $prep_field['appointment_start_date'] = $Lab_appointment_start_date;
       $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['labels']['appointment_start_date'] = $Lab_appointment_start_date; 
       $prep_field['current_status_id'] = $Lab_current_status_id;
       $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['labels']['current_status_id'] = $Lab_current_status_id; 
       $Str_gb = "";
       foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['SC_Gb_Free_cmp'] as $cmp_gb => $resto)
       {
          $Str_gb .= ($Str_gb == "") ? "" : ",";
       ...
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['labels']['appointments_id']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['labels']['appointments_id'] = "" . $this->Ini->Nm_lang['lang_appointments_fld_appointments_id'] . ""; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['labels']['staff_id']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['labels']['staff_id'] = "" . $this->Ini->Nm_lang['lang_appointments_fld_staff_id'] . ""; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['labels']['customers_id']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['labels']['customers_id'] = "" . $this->Ini->Nm_lang['lang_appointments_fld_customers_id'] . ""; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['labels']['price']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['labels']['price'] = "" . $this->Ini->Nm_lang['lang_appointments_fld_price'] . ""; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['labels']['additional_charges']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['labels']['additional_charges'] = "" . $this->Ini->Nm_lang['lang_appointments_fld_additional_charges'] . ""; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['labels']['discount']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['labels']['discount'] = "" . $this->Ini->Nm_lang['lang_appointments_fld_discount'] . ""; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['labels']['total_price']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['labels']['total_price'] = "" . $this->Ini->Nm_lang['lang_appointments_fld_total_price'] . ""; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['labels']['appointment_start_date']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['labels']['appointment_start_date'] = "" . $this->Ini->Nm_lang['lang_appointments_fld_appointment_start_date'] . ""; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['labels']['appointment_start_time']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['labels']['appointment_start_time'] = "" . $this->Ini->Nm_lang['lang_appointments_fld_appointment_start_time'] . ""; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['labels']['appointment_end_date']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['labels']['appointment_end_date'] = "" . $this->Ini->Nm_lang['lang_appointments_fld_appointment_end_date'] . ""; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['labels']['appointment_end_time']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['labels']['appointment_end_time'] = "" . $this->Ini->Nm_lang['lang_appointments_fld_appointment_end_time'] . ""; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['labels']['current_status_id']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['labels']['current_status_id'] = "" . $this->Ini->Nm_lang['lang_appointments_fld_current_status_id'] . ""; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['labels']['appointment_recurrent']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['labels']['appointment_recurrent'] = "" . $this->Ini->Nm_lang['lang_appointments_fld_appointment_recurrent'] . ""; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['labels']['appointment_period']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['labels']['appointment_period'] = "" . $this->Ini->Nm_lang['lang_appointments_fld_appointment_period'] . ""; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['labels']['appointment_rating']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['labels']['appointment_rating'] = "" . $this->Ini->Nm_lang['lang_appointments_fld_appointment_rating'] . ""; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['labels']['intervalo']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['labels']['intervalo'] = "Intervalo"; 
       }
       //-----
       $ix = 0;
       $this->comp_field_nm = array();
       ...
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['summarizing_fields_order']['sc_free_group_by']))
       {
           foreach ($this->comp_sum as $i_sum => $l_sum)
           {
               $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['summarizing_fields_order']['sc_free_group_by'][] = $i_sum;
           }
       }
       else
       {
           $this->comp_sum_order = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['summarizing_fields_order']['sc_free_group_by'];
       }
       ...
       $this->comp_sum_display = array(
           ...
           foreach ($this->comp_sum as $i_sum => $l_sum)
           {
               if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['summarizing_fields_display']['sc_free_group_by'][$i_sum]))
               {
                   $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['summarizing_fields_display']['sc_free_group_by'][$i_sum] = array('label' => $l_sum, 'display' => $this->comp_sum_display[$i_sum]);
               }
               else
               {
                   if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['summarizing_fields_display']['sc_free_group_by'][$i_sum]['label']))
                   {
                       $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['summarizing_fields_display']['sc_free_group_by'][$i_sum]['label'] = $l_sum;
                   }
                   if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['summarizing_fields_display']['sc_free_group_by'][$i_sum]['display']))
                   {
                       $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['summarizing_fields_display']['sc_free_group_by'][$i_sum]['display'] = $this->comp_sum_display[$i_sum];
                   }
               }
           }
           foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['summarizing_fields_display']['sc_free_group_by'] as $i_sum => $d_sum)
           {
       ...
       //-----
       $this->comp_sum_lnk = array(
           2 => array('field' => "intervalo", 'show' => true),
       );
      ...
      eval ($Str_gb);
       ...
       $Arr_parms = array();
       $Arr_parms['appointment_start_date']['alin'] = "''";
       $Arr_parms['appointment_start_date']['link'] = "S";
       $Arr_parms['current_status_id']['alin'] = "''";
       $Arr_parms['current_status_id']['link'] = "S";
       $Arr_parms['appointment_start_date']['fill'] = "true";
       $Arr_parms['appointment_start_date']['order'] = "value";
       $Arr_parms['appointment_start_date']['order_start'] = 'asc';
       $Arr_parms['appointment_start_date']['order_invert'] = 'false';
       $Arr_parms['appointment_start_date']['order_enabled'] = 'false';
       $Arr_parms['appointment_start_date']['order_datatype'] = 'date';
       $Arr_parms['appointment_start_date']['rating_function'] = '';
       $Arr_parms['current_status_id']['fill'] = "true";
       $Arr_parms['current_status_id']['order'] = "value";
       $Arr_parms['current_status_id']['order_start'] = 'asc';
       $Arr_parms['current_status_id']['order_invert'] = 'false';
       $Arr_parms['current_status_id']['order_enabled'] = 'false';
       $Arr_parms['current_status_id']['order_datatype'] = 'int';
       $Arr_parms['current_status_id']['rating_function'] = '';
       //-----
       $Str_gb = "";
       foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['SC_Gb_Free_cmp'] as $cmp_gb => $resto)
       {
       ...
       //-----
       $prep_links_fl = array();
       $prep_links_fl['appointment_start_date'] = array(0 => 'appointment_start_date', 1 => '@aspass@');
       $prep_links_fl['current_status_id'] = array(0 => 'current_status_id', 1 => '');
       $Str_gb = "";
       foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['SC_Gb_Free_cmp'] as $cmp_gb => $col_sql)
       {
       ...
       //-----
       $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['pivot_group_by'] = $this->comp_field;
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['pivot_x_axys']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['pivot_x_axys'] = $this->comp_x_axys;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['pivot_y_axys']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['pivot_y_axys'] = $this->comp_y_axys;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['pivot_fill']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['pivot_fill'] = $this->comp_fill;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['pivot_order']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['pivot_order'] = $this->comp_order;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['pivot_order_start']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['pivot_order_start'] = $this->comp_order_start;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['pivot_tabular']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['pivot_tabular'] = $this->comp_tabular;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['pivot_tabular_hover']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['pivot_tabular_hover'] = $this->comp_tabular && $this->comp_tab_hover;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['pivot_tabular_seq']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['pivot_tabular_seq'] = $this->comp_tabular && $this->comp_tab_seq;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['pivot_tabular_label']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['pivot_tabular_label'] = $this->comp_tabular && $this->comp_tab_label;
       }

       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['pivot_order_col']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['pivot_order_col'] = 0;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['pivot_order_level']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['pivot_order_level'] = 0;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['pivot_order_sort']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['pivot_order_sort'] = '';
       }
       if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['ajax_nav'] && isset($_POST['parm']))
       { 
       ...
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['pivot_order_sort'] = $_POST['sort_ord'];
           if ('' == $_POST['sort_ord'])
           {
               $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['pivot_order_col']  = 0;
           }
           ...
           {
               $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['pivot_order_col']  = $_POST['sort_col'];
           }
       }
       if (isset($_POST['change_sort']) && 'NEW' == $_POST['change_sort']) {
           for ($i = 0; $i < sizeof($this->comp_label); $i++) {
               if ($i == $_POST['sort_col']) {
                   $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['pivot_order_start'][$i] = $_POST['sort_ord'];
               }
           }
       }

       $this->comp_x_axys      = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['pivot_x_axys'];
       $this->comp_y_axys      = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['pivot_y_axys'];
       $this->comp_fill        = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['pivot_fill'];
       $this->comp_order       = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['pivot_order'];
       $this->comp_order_start = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['pivot_order_start'];
       $this->comp_order_col   = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['pivot_order_col'];
       $this->comp_order_level = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['pivot_order_level'];
       $this->comp_order_sort  = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['pivot_order_sort'];
       $this->comp_tabular     = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['pivot_tabular'];
       $this->comp_tab_hover   = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['pivot_tabular_hover'];
       $this->comp_tab_seq     = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['pivot_tabular_seq'];
       $this->comp_tab_label   = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['pivot_tabular_label'];
   ...
   function getLabelLink($param, $i_tmp = -1, $bProtect = true)
   {
       ...
       $Md5_Res    = "@SC_par@" . NM_encode_input($this->Ini->sc_page) . "@SC_par@chart_appointments_count@SC_par@" . md5($Parms_Res);
       $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['LigR_Md5'][md5($Parms_Res)] = $Parms_Res;
       return $Md5_Res;
   }
   ...
   function makeTabular(&$a_data)
   {
       ...
       {
           $a_labels = array();
   if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['proc_pdf'] || $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['proc_pdf_vert'])
   {
   ...
   function buildChart()
   {
       ...
       $prep_chart_config = array(
           ...
           '1' => array(
               ...
               'show_sub' => true,
               'subtitle' => "",
               'field_x'  => 'appointment_start_date',
               'field_y'  => $this->comp_sum_nm[2],
               'label_x'  => $this->getChartText(strip_tags($Lab_appointment_start_date)),
           ...
           '1' => array(
               ...
               'show_sub' => true,
               'subtitle' => "",
               'field_x'  => 'current_status_id',
               'field_y'  => $this->comp_sum_nm[2],
               'label_x'  => $this->getChartText(strip_tags($Lab_current_status_id)),
               'label_y'  => $this->getChartText(strip_tags("Intervalo (" .  $this->Ini->Nm_lang['lang_btns_smry_msge_sum'] . ")")),
               'format'   => $this->formatChartValue(1),
           ),
           ),
       ...
       foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['summarizing_fields_order']['sc_free_group_by'] as $i_summ_order)
       {
                   ...
                   foreach ($this->sc_comb_quick_index[$i_index_summ] as $i_index_comb_groupby)
                   {
                       ...
                       if (!isset($a_chart_combinations[$s_comb_index]))
                       {
                           $a_chart_combinations[$s_comb_index] = array(
                               ...
                               'label_x'     => '',
                               'label_y'     => '',
                               'xml_pholder' => sizeof($this->comp_y_axys) > $i_index_group + 1 ? 'sc_chart_appointments_count_' . session_id() . '_!!!C_' . ($i_index_group + 1) . '_' . $s_index_comb . ('' == $s_index_comb ? '' : '_') . '__SC_PLACEHOLDER__!!!.json' : '',
                               'xml'         => 'sc_chart_appointments_count_' . session_id() . '_!!!C_' . $i_index_group . '_' . $s_index_comb . '!!!.json',
                           );
                       }
       ...
       $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['pivot_charts']    = $this->comp_chart_data;
       $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['pivot_orig']      = $this->comp_chart_data;
       $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['comb_table_data'] = array();
       $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['comb_table_def']  = array();
       $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['anlt_table_data'] = array();
       $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['anlt_table_def']  = array();
	...
	function filterChartsBySummarization() {
		...
		if (isset($nmgp_opcao) && ('pdf' == $nmgp_opcao || 'pdf_res' == $nmgp_opcao)) {
			...
			foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['summarizing_fields_display']['sc_free_group_by'] as $fieldIndex => $fieldData) {
				if ($fieldData['display']) {
					$displayedSummarizations[] = $fieldIndex;
				}
			}

			foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['pivot_charts'] as $chartIndex => $chartData) {
				...
				if (!in_array($summarizingLevel + 1, $displayedSummarizations)) {
					unset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['pivot_charts'][$chartIndex]);
				}
			}

			$this->comp_chart_data = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['pivot_charts'];
		}
	}
	...
	function filterChartsByMultiseries() {
		...
		if (isset($nmgp_opcao) && ('pdf' == $nmgp_opcao || 'pdf_res' == $nmgp_opcao) && isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['chart_level']) && '' !== $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['chart_level']) {
			$applyFilter = true;
		}
		...
		if ($applyFilter && (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['summarizing_drill_down']) || !$_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['summarizing_drill_down'])) {
			foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['pivot_charts'] as $chartIndex => $chartData) {
				$indexParts = explode('|', $chartIndex);
           ...
           foreach ($a_totals as $i_total => $values)
           {
               ...
               if (isset($this->comp_chart_config[$key_config]))
               {
                   ...
                   foreach ($a_keys as $tmp_key)
                   {
                       ...
                       $xml_links[]  = $this->getFusionLink('sc_chart_appointments_count_' . session_id() . '_!!!' . implode('_', array_merge(array($group + 1, $i_total), $link_index)) . '!!!.json');
                   }
                   ...
                   $this->comp_chart_data[$key_data]['xml']        = $this->getFusionLink('sc_chart_appointments_count_' . session_id() . '_!!!' . implode('_', array_merge(array($group, $i_total), $param)) . '!!!.json');
   ...
   function drawMatrix()
   {
       ...
       if ($this->NM_export)
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['arr_export']['label'] = $this->build_labels;
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['arr_export']['data']  = $this->build_data;
           return;
       }
      ...
      if ($this->comp_tab_hover)
      {
       ...
       foreach ($this->build_data as $row_i => $lines)
       {
           ...
           foreach ($lines as $col_i => $columns)
           {
               ...
               {
                   if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['opcao'] != "pdf" && $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['opcao'] != "print" && !$_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['embutida'])
                   {
           ...
           if (!$b_display)
           {
               if ($this->comp_tabular)
               {
                   ...
                   foreach ($this->comp_y_axys as $iYAxysIndex)
                   {
                       ...
                       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['pivot_order_start'][$iYAxysIndex]))
                       {
                           $sInitialOrder   = '';
                           $sInitialDisplay = '; display: none';
                           $sInitialSrc     = '';
                       }
                       elseif ('asc' == $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['pivot_order_start'][$iYAxysIndex])
                       {
...
if($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['proc_pdf'] || $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['proc_pdf_vert']){ 
   ...
   function resumo_init()
   {
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['opcao'] != "pdf" && !$_SESSION['scriptcase']['proc_mobile'])
      {
   ...
   function monta_css()
   {
      global $nm_saida, $nmgp_tipo_pdf, $nmgp_cor_print;
       ...
       if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['embutida'])
       {
          if (($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['opcao'] == "print" && strtoupper($nmgp_cor_print) == "PB") || $nmgp_tipo_pdf == "pb")
           { 
               if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['SC_sub_css_bw']['chart_appointments_count']))
               {
                   $compl_css = str_replace(".", "_", $_SESSION['sc_session'][$this->Ini->sc_page]['SC_sub_css_bw']['chart_appointments_count']) . "_";
               } 
           } 
           else 
           { 
               if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['SC_sub_css']['chart_appointments_count']))
               {
                   $compl_css = str_replace(".", "_", $_SESSION['sc_session'][$this->Ini->sc_page]['SC_sub_css']['chart_appointments_count']) . "_";
               } 
           }
       }
   ...
   function resumo_final()
   {
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['opcao'] != "pdf" && !$_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['ajax_nav'])
      {
   ...
   {
      $this->Tot_ger = false;
      require_once($this->Ini->path_aplicacao . "chart_appointments_count_total.class.php"); 
      $this->Print_All = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['print_all'];
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['doc_word'] || $this->Ini->sc_export_ajax_img)
      { 
      ...
      $this->Total   = new chart_appointments_count_total($this->Ini->sc_page);
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['LigR_Md5'] = array();
   }
   ...
   function totaliza()
   {
      ...
      foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['arr_total'] as $cmp_gb => $resto)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $this->$Arr_tot_name = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['arr_total'][$cmp_gb];
      }
...
if (!$_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['responsive_chart']['active']) {
$nm_saida->saida("<!DOCTYPE HTML PUBLIC \"-//W3C//DTD HTML 4.01 Transitional//EN\"\r\n");
$nm_saida->saida("            \"http://www.w3.org/TR/1999/REC-html401-19991224/loose.dtd\">\r\n");
}
$_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['dashboard_refresh_after_chart'] = 'resumo';
      $nm_saida->saida("<HTML" . $_SESSION['scriptcase']['reg_conf']['html_dir'] . ">\r\n");
      $nm_saida->saida("<HEAD>\r\n");
      $nm_saida->saida(" <TITLE>" . $this->Ini->Nm_lang['lang_othr_chart_titl'] . " - appointments</TITLE>\r\n");
...
if (!$_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['doc_word'])
{
      ...
      { 
           ...
           $nm_saida->saida(" <script type=\"text/javascript\" src=\"chart_appointments_count_ajax.js\"></script>\r\n");
           $nm_saida->saida(" <script type=\"text/javascript\">\r\n");
           ...
           $nm_saida->saida(" </script>\r\n");
           $nm_saida->saida(" <script type=\"text/javascript\" src=\"chart_appointments_count_jquery-3.6.0.min.js\"></script>\r\n");
           $nm_saida->saida(" <script type=\"text/javascript\" src=\"chart_appointments_count_message.js\"></script>\r\n");
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['num_css']))
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['num_css'] = rand(0, 1000);
      }
      $NM_css = @fopen($this->Ini->root . $this->Ini->path_imag_temp . '/sc_css_chart_appointments_count_sum_' . $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['num_css'] . '.css', 'w');
      if (($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['opcao'] == "print" && strtoupper($nmgp_cor_print) == "PB") || $nmgp_tipo_pdf == "pb")
      {
      ...
      @fclose($NM_css);
     $this->Ini->summary_css = $this->Ini->path_imag_temp . '/sc_css_chart_appointments_count_sum_' . $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['num_css'] . '.css';
     if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['opcao'] == "print")
     {
         $nm_saida->saida("  <style type=\"text/css\">\r\n");
         $NM_css = file($this->Ini->root . $this->Ini->path_imag_temp . '/sc_css_chart_appointments_count_sum_' . $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['num_css'] . '.css');
     ...
     $nm_saida->saida("   <link rel=\"stylesheet\" type=\"text/css\" href=\"" . $this->Ini->path_link . "chart_appointments_count/chart_appointments_count_res_" . strtolower($_SESSION['scriptcase']['reg_conf']['css_dir']) . ".css\" />\r\n");
     $nm_saida->saida(" <style type=\"text/css\">\r\n");
...
if ($_SESSION['scriptcase']['proc_mobile'])
{
      ...
      if (!$this->Ini->Export_html_zip && $this->Print_All && !$_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['doc_word']) 
      {
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['opcao'] != "print" && !$_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['embutida'])
      {
      ...
      $nm_saida->saida("function scChartPrintExportShow(exportType) {\r\n");
          $nm_saida->saida("  scShowCombinationPrint(0, exportType, \"bw\");\r\n");
      $nm_saida->saida("}\r\n");
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['opcao'] != "pdf" && !$_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['embutida'] && !$_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['doc_word'])
      { 
          $nm_saida->saida("      <STYLE>\r\n");
   ...
   function monta_barra_top_normal()
   {
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['opcao'] != "print" && !$_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['embutida'])
      {
         ...
         if ($this->nmgp_botoes['chart_summary'] == 'on') {
            $Cod_Btn = nmButtonOutput($this->arr_buttons, "bcresumo", "scShowCombinationTable();", "scShowCombinationTable();", "chart_table_top", "", "", "", "absmiddle", "", "0px", $this->Ini->path_botoes, "", "", "", "", "", "only_text", "text_right", "", "", "", "", "", "", "");
            $nm_saida->saida("     $Cod_Btn\r\n");
            $NM_btn = true;
         }
         $nm_saida->saida("     </TD> \r\n");
         ...
         if ($this->nmgp_botoes['chart_type'] == 'on') {
            $Cod_Btn = nmButtonOutput($this->arr_buttons, "bctype", "scShowChartSelection('#chart_type_top');", "scShowChartSelection('#chart_type_top');", "chart_type_top", "", "", "", "absmiddle", "", "0px", $this->Ini->path_botoes, "", "", "", "", "", "only_text", "text_right", "", "", "", "", "", "", "");
            $nm_saida->saida("     $Cod_Btn\r\n");
            $NM_btn = true;
         }
      ...
      if ($this->nmgp_botoes['filter'] == 'on' && !$this->Ini->SC_Link_View && !$this->grid_emb_form)
      {
         ...
         if (is_file("chart_appointments_count_help.txt") && !$this->grid_emb_form && !$this->NM_res_sem_reg)
         {
            $Arq_WebHelp = file("chart_appointments_count_help.txt"); 
...
$nm_saida->saida("       scJs_alert('" . $mensagem . "', $jsonParams);\r\n");
      ...
      elseif ($this->Ini->Export_html_zip || ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['opcao'] == "pdf" && $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['graf_pdf'] == "S"))
      {
          $this->grafico_pdf();
      }
      else
...
{ 
      ...
      $nm_saida->saida("</TABLE>\r\n");
       ...
       $nm_saida->saida("  tb_show('', 'chart_appointments_count_config_pivot.php?nome_apl=chart_appointments_count&sc_page=" . NM_encode_input($this->Ini->sc_page) . "&language=es&TB_iframe=true&modal=true&height=300&width=500', '');\r\n");
       $nm_saida->saida("}\r\n");
   ...
   $nm_saida->saida("                  action=\"chart_appointments_count_iframe_prt.php\" \r\n");
   ...
   $nm_saida->saida("<SCRIPT language=\"Javascript\">\r\n");
          ...
          $nm_saida->saida("               nm_submit_modal(\"" . $this->Ini->path_link . "chart_appointments_count/chart_appointments_count_export_email.php?script_case_init={$this->Ini->sc_page}&path_img={$this->Ini->path_img_global}&path_btn={$this->Ini->path_botoes}&sType=\"+ str_type +\"&sAdd=__E__cor_print=\" + cor + \"__E__SC_module_export=\" + SC_module_export + \"__E__nmgp_password=\" + password + \"&KeepThis=true&TB_iframe=true&modal=true\", bol_param);\r\n");
          $nm_saida->saida("       }\r\n");
          $nm_saida->saida("       else\r\n");
          $nm_saida->saida("       {\r\n");
          ...
          $nm_saida->saida("       document.Fexport.action = \"chart_appointments_count_export_ctrl.php\";\r\n");
      ...
      if (!extension_loaded("zip"))
      {
          ...
          $nm_saida->saida("       document.F3.SC_module_export.value = z;\r\n");
          $nm_saida->saida("  }\r\n");
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['chart_appointments_count_iframe_params'] = array(
      ...
      $Md5_pdf    = "@SC_par@" . NM_encode_input($this->Ini->sc_page) . "@SC_par@chart_appointments_count@SC_par@" . md5($prep_parm_pdf);
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['Md5_pdf'][md5($prep_parm_pdf)] = $prep_parm_pdf;
          ...
          $nm_saida->saida("          document.Fres_pdf.action = \"chart_appointments_count_iframe.php\";\r\n");
...
;
   ...
   function monta_html_ini_pdf()
   {
      global $nm_saida;
       $tp_quebra = "";
       if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['num_css']) && !empty($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['num_css']))
       {
           $NM_css = @fopen($this->Ini->root . $this->Ini->path_imag_temp . '/sc_css_chart_appointments_count_grid_' . $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['num_css'] . '.css', 'a');
           $NM_css_file = $this->Ini->root . $this->Ini->path_link . "chart_appointments_count/chart_appointments_count_res_" . strtolower($_SESSION['scriptcase']['reg_conf']['css_dir']). ".css";
       ...
       if ($this->Print_All || $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['print_all'])
       {
       $tp_quebra = "<div style=\"page-break-after: always;\"><span style=\"display: none;\">&nbsp;</span></div>";
       }
...
//--- 
 function grafico_pdf()
 {
   ...
   $this->Graf->Ini    = $this->Ini;
   $this->Graf->Lookup = $this->Lookup;
   if (!empty($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['pivot_charts']) && (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['skip_charts']) || !$_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['skip_charts']))
   {
 ...
 { 
       ...
       foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['pivot_charts'] as $chart_index => $chart_data)
       {
 ...
 { 
           ...
           if ($this->progress_fp)
           {
               chart_appointments_count_pdf_progress_call($this->progress_tot . "_#NM#_" . $this->progress_now . "_#NM#_" . $sChartLang . " " . $iChartCount . "...\n", $this->Ini->Nm_lang, true);
           ...
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['this_chart_label'] = '';
           $this->Graf->monta_grafico($chart_index, 'pdf');
           if ('' != $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['this_chart_label']) {
               $nm_saida->saida("<script type=\"text/javascript\">$(\"#$random_id\").html(\"{$_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['this_chart_label']}\")</script>\r\n");
           }
       ...
       if ($bChartFP)
       {
           ...
           chart_appointments_count_pdf_progress_call(100 . "_#NM#_" . 90 . "_#NM#_" . $lang_protect . "...\n", $this->Ini->Nm_lang);
           fwrite($this->progress_fp, 90 . "_#NM#_" . $lang_protect . "...\n");
           fclose($this->progress_fp);
       }
       if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['charts_html']) && '' != $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['charts_html'])
       {
            $nm_saida->saida("<script type=\"text/javascript\">\r\n");
            $nm_saida->saida("{$_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['charts_html']}\r\n");
            $nm_saida->saida("</script>\r\n");
       }
   }
       $nm_saida->saida("</body>\r\n");
...
//--- 
 ...
 {
   global $nm_saida, $nm_lang;
   if (!empty($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['chart_list']))
   {
       $prim_graf = true;
       foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['chart_list'] as $arr_chart)
       {
           if (!$prim_graf)
           {
               $nm_saida->saida("<div style=\"page-break-after: always;\"><span style=\"display: none;\">&nbsp;</span></div>\r\n");
...
//--- 
 function grafico_bot()
 {
   ...
   if (!empty($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['pivot_charts']))
   {
       $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['graf_bot'] = true;
       $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['graf_first'] = true;
       require_once($this->Ini->path_aplicacao . $this->Ini->Apl_grafico); 
       $this->Graf  = new chart_appointments_count_grafico();
       $this->Graf->Db     = $this->Db;
       $this->Graf->Erro   = $this->Erro;
       ...
       $nm_saida->saida("<TR>\r\n");
       $iChartCount = 1;
       $iChartTotal = sizeof($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['pivot_charts']);
       $sChartLang  = isset($this->Ini->Nm_lang['lang_pdff_pcht']) ? $this->Ini->Nm_lang['lang_pdff_pcht'] : 'Generating chart';
       if (!NM_is_utf8($sChartLang))
       {
           $sChartLang = sc_convert_encoding($sChartLang, "UTF-8", $_SESSION['scriptcase']['charset']);
      ...
      if ($bPDF && (!isset($this->progress_fp) || !$this->progress_fp))
      {
           ...
           chart_appointments_count_pdf_progress_call("PDF\n", $this->Ini->Nm_lang);
           chart_appointments_count_pdf_progress_call("\n", $this->Ini->Nm_lang);
           chart_appointments_count_pdf_progress_call("\n", $this->Ini->Nm_lang);
           chart_appointments_count_pdf_progress_call("100\n", $this->Ini->Nm_lang);
       ...
       foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['pivot_charts'] as $chart_index => $chart_data)
       {
           ...
           if ($bPDF)
           {
               ...
               if ($this->progress_fp)
               {
                   chart_appointments_count_pdf_progress_call($this->progress_tot . "_#NM#_" . $this->progress_now . "_#NM#_" . $sChartLang . " " . $iChartCount . "/" . $iChartTotal . "...\n", $this->Ini->Nm_lang, true);
       ...
       $nm_saida->saida("</TD>\r\n");
       $nm_saida->saida("</TR>\r\n");
       $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['graf_bot'] = false;
       if ($bPDF && isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['charts_html']) && '' != $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['charts_html'])
       {
           $nm_saida->saida("<script type=\"text/javascript\">\r\n");
           $nm_saida->saida("{$_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['charts_html']}\r\n");
           $nm_saida->saida("</script>\r\n");
...
//--- 
   ...
   {
      global $nm_saida;
      if ($this->Ini->Embutida_iframe || $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['embutida'] || $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['dashboard_info']['compact_mode'])
      { 
          return;
      } 
      ...
      { 
        ...
        if ($_SESSION['scriptcase']['charset'] != "UTF-8")
        {
          ...
          $appointment_start_date = (isset($Busca_temp['appointment_start_date'])) ? $Busca_temp['appointment_start_date'] : ""; 
          $tmp_pos = (is_string($appointment_start_date)) ? strpos($appointment_start_date, "##@@") : false;
          if ($tmp_pos !== false && !is_array($appointment_start_date))
          {
              $appointment_start_date = substr($appointment_start_date, 0, $tmp_pos);
          }
          $appointment_start_date_2 = (isset($Busca_temp['appointment_start_date_input_2'])) ? $Busca_temp['appointment_start_date_input_2'] : ""; 
      } 
      if (!empty($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['cond_pesq']))
      {  
          $pos       = 0;
          $trab_pos  = false;
          $pos_tmp   = true; 
          $tmp       = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['cond_pesq'];
          while ($pos_tmp)
          {
             $pos = strpos($tmp, "##*@@", $pos);
             if ($pos !== false)
          ...
          $nm_cond_filtro_or  = (substr($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['cond_pesq'], $trab_pos + 5) == "or")  ? " " . trim($this->Ini->Nm_lang['lang_srch_orr_cond']) . " " : "";
          $nm_cond_filtro_and = (substr($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['cond_pesq'], $trab_pos + 5) == "and") ? " " . trim($this->Ini->Nm_lang['lang_srch_and_cond']) . " " : "";
          $nm_cab_filtro   = substr($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['cond_pesq'], 0, $trab_pos);
     ...
     if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['proc_pdf'] || $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['proc_pdf_vert']) {
      ...
      $nm_saida->saida("    	<tr>\r\n");
      $nm_saida->saida("        	<td id=\"lin1_col1\" class=\"" . $this->css_scGridHeaderFont . "\"><span>" . $this->Ini->Nm_lang['lang_othr_chart_titl'] . " - appointments</span></td>\r\n");
   ...
   //---- 
   function adiciona_registro($sum_intervalo, $quebra_appointment_start_date, $quebra_current_status_id, $quebra_appointment_start_date_orig, $quebra_current_status_id_orig)
   {
      $contr_arr = "";
      foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['SC_Gb_Free_cmp'] as $cmp_gb => $resto)
      {
          $Name_orig  = (isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['SC_Gb_Free_orig'][$cmp_gb])) ? $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['SC_Gb_Free_orig'][$cmp_gb] : $cmp_gb;
          ...
          $cmp_look   = "Cmp_formt";
          $cmp_orig   = "Cmp_arg";
          if (empty($sum_intervalo)) {$sum_intervalo = '0';}
          eval ('
          if (!isset($this->' . $arr_name . '))
          {
              $this->' . $arr_name . '[0] = 1;
              $this->' . $arr_name . '[1] = ' . $sum_intervalo . ';
          ...
          {
              $this->' . $arr_name . '[0]++;
              $this->' . $arr_name . '[1] = bcadd($this->' . $arr_name . '[1], ' . $sum_intervalo . ', 10);
          }
          ');
      }
   }
...
//
   function nm_acumula_resumo($nm_tipo="resumo")
   {
     ...
     { 
       ...
       $this->appointment_start_date = $Busca_temp['appointment_start_date']; 
       $this->appointment_start_date = (isset($Busca_temp['appointment_start_date'])) ? $Busca_temp['appointment_start_date'] : ""; 
       $tmp_pos = (is_string($this->appointment_start_date)) ? strpos($this->appointment_start_date, "##@@") : false;
       if ($tmp_pos !== false && !is_array($this->appointment_start_date))
       {
     ...
     { 
         $nmgp_select = "SELECT appointments_id, staff_id, customers_id, price, additional_charges, discount, total_price, appointment_start_date, appointment_start_time, appointment_end_date, appointment_end_time, current_status_id, appointment_recurrent, appointment_period, appointment_rating, intervalo from " . $this->Ini->nm_tabela; 
     } 
     else 
     { 
         $nmgp_select = "SELECT appointments_id, staff_id, customers_id, price, additional_charges, discount, total_price, appointment_start_date, appointment_start_time, appointment_end_date, appointment_end_time, current_status_id, appointment_recurrent, appointment_period, appointment_rating, intervalo from " . $this->Ini->nm_tabela; 
     } 
     $nmgp_select .= " " . $_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['where_pesq']; 
     $campos_order = "";
     foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_appointments_count']['SC_Gb_Free_sql'] as $cmp_var => $resto)
     {
...
// 
   function nm_acum_res_unit($rs_res, $nm_tipo="resumo")
   {
            ...
            $this->appointments_id = $rs_res->fields[0] ;  
            $this->staff_id = $rs_res->fields[1] ;  
            $this->customers_id = $rs_res->fields[2] ;  
            $this->price = $rs_res->fields[3] ;  
            $this->price =  str_replace(",", ".", $this->price);
            $this->additional_charges = $rs_res->fields[4] ;  
            $this->additional_charges =  str_replace(",", ".", $this->additional_charges);
            $this->discount = $rs_res->fields[5] ;  
            $this->discount =  str_replace(",", ".", $this->discount);
            $this->total_price = $rs_res->fields[6] ;  
            $this->total_price =  str_replace(",", ".", $this->total_price);
            $this->appointment_start_date = $rs_res->fields[7] ;  
            $this->appointment_start_time = $rs_res->fields[8] ;  
            $this->appointment_end_date = $rs_res->fields[9] ;  
            $this->appointment_end_time = $rs_res->fields[10] ;  
            $this->current_status_id = $rs_res->fields[11] ;  
            $this->appointment_recurrent = $rs_res->fields[12] ;  
            $this->appointment_period = $rs_res->fields[13] ;  
            $this->appointment_rating = $rs_res->fields[14] ;  
            $this->intervalo = $rs_res->fields[15] ;  
            $this->intervalo =  str_replace(",", ".", $this->intervalo);
            $this->appointment_start_date_orig = $this->appointment_start_date;
            $this->current_status_id_orig = $this->current_status_id;
            if ($nm_tipo == "resumo")
            {
                $this->adiciona_registro($this->intervalo, $this->appointment_start_date, $this->current_status_id, $this->appointment_start_date_orig,  $this->current_status_id_orig);
            }
   }
//
