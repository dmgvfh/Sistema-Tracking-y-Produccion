...
class grid_appointments_adm_total
{
   var $Db;
   var $Erro;
   var $Ini;
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['grid_appointments_adm']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['grid_appointments_adm']['campos_busca']))
      { 
   ...
   function Calc_resumo_status($destino_resumo, $res_export=false)
   {
      global $nm_lang;
      $this->nm_data = new nm_data("es");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_adm']['sql_tot_res']);
      ...
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_adm']['SC_Ind_Groupby'];
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_adm']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_adm']['arr_total'][$cmp_gb] = $this->$Arr_tot_name;
      }
   }
   ...
   function Res_Totaliza_status($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def, $res_export)
   {
     ...
     $comando .= " ON " . $join;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_adm']['sql_tot_res']))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_adm']['sql_tot_res'] = str_replace("#@#cmps_quebras#@#", "", $comando);
      }
      $comando  = str_replace("#@#cmps_quebras#@#", "," . $cmps_quebras, $comando);
      $comando .= " group by " . $cmps_quebras . " order by " .  $cmps_quebras;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_adm']['Res_search_metric_use']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_adm']['Res_search_metric_use']))
      {
          $comando = $cmd_simp;
          $cmps_S  = "";
   ...
   function Calc_resumo_sc_free_group_by($destino_resumo, $res_export=false)
   {
      ...
      $this->sc_where_atual  = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_adm']['where_pesq'];
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_adm']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('appointments_appointment_start_date' => "appointments.appointment_start_date",'appointments_current_status_id' => "appointments.current_status_id",'nombre_usuario' => "sec_users.name",'appointments_id_proyecto' => "appointments.id_proyecto");
      $cmps_quebra_atual = array();
      foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_adm']['SC_Gb_Free_cmp'] as $cmp => $resto)
      {
   ...
   function Res_Totaliza_sc_free_group_by($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def, $res_export)
   {
     ...
     $comando .= " ON " . $join;
      ...
      $format_dimensions['appointments_id_proyecto']['reg'] = "S";
      $format_dimensions['appointments_id_proyecto']['msk'] = "";
      while (!$rt->EOF)
      {
          $sql_where = "";
          foreach ($Cmp_dim as $Cada_dim => $Ind_sql)
          {
              ...
              if ($Cada_dim == "appointments_id_proyecto") {
                  $this->Lookup->lookup_sc_free_group_by_appointments_id_proyecto($$Cada_dim,  $appointments_id_proyecto);
              }
              if (null === $$Cada_dim)
              {
                  $$Cada_dim = '';
   ...
   function quebra_geral_status($res_limit=false, $res_export=false)
   {
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_adm']['contr_total_geral'] == "OK") 
      { 
          return; 
      } 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_adm']['tot_geral'] = array() ;  
      $nm_comando = "select count(*), sum(appointments.intervalo) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_adm']['where_pesq']; 
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_adm']['tot_geral'][0] = "Total Horas"; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_adm']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $rt->fields[1] = (string)$rt->fields[1]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_adm']['tot_geral'][2] = $rt->fields[1]; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_adm']['contr_total_geral'] = "OK";
   } 
   ...
   function quebra_appointments_id_proyecto_sc_free_group_by($appointments_id_proyecto, $Where_qb, $Cmp_Name) 
   {
      $Var_name_gb = "SC_tot_" . $Cmp_Name;
      global $$Var_name_gb, $appointments_current_status_id;
      $tot_appointments_id_proyecto = array() ;  
      $nm_comando = "select count(*), sum(appointments.intervalo) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_adm']['where_pesq']; 
      if (empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_adm']['where_pesq'])) 
      { 
         $nm_comando .= " where " . $Where_qb ; 
      ...
      $tot_appointments_id_proyecto[0] = NM_encode_input(sc_strip_script($appointments_id_proyecto)) ; 
      $tot_appointments_id_proyecto[1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $tot_appointments_id_proyecto[2] = (string)$rt->fields[1]; 
      $rt->Close(); 
      $$Var_name_gb = $tot_appointments_id_proyecto;
   } 
