...
class grid_prod_presupuesto_res_xml
{
   var $Db;
   var $Erro;
   var $Ini;
   ...
   function grava_arquivo()
   {
      ...
      if ($this->New_Format)
      {
          fwrite($xml_f, "<grid_prod_presupuesto>\r\n");
      }
      if ($this->Grava_view)
      {
          ...
          if ($this->New_Format)
          {
              fwrite($xml_v, "<grid_prod_presupuesto>\r\n");
          }
      }
      ...
      $this->array_titulos = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_prod_presupuesto']['arr_export']['label'];
      $this->array_linhas  = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_prod_presupuesto']['arr_export']['data'];
      $contr_rowspan = array();
      $tit_rowspan   = array();
   ...
   function grava_linha($xml_f)
   {
      ...
      foreach ($this->array_linhas as $lines)
      {
          ...
          if (!$this->New_Format)
          {
              $xml_registro = "<grid_prod_presupuesto";
          }
          foreach ($lines as $columns)
          {
              if (0 <= $columns['level'])
