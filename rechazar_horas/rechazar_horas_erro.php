...
$old_error_handler = set_error_handler("rechazar_horas_scriptcase_error_handler");

function rechazar_horas_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe="", $err_usr=false)
{
    ...
    if (isset($_SESSION['scriptcase']['rechazar_horas']['contr_erro']) && $_SESSION['scriptcase']['rechazar_horas']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_rechazar_horas->contr_rechazar_horas->Apl_com_erro = true;
            if ($inicial_rechazar_horas->contr_rechazar_horas->NM_ajax_flag)
            {
                $inicial_rechazar_horas->contr_rechazar_horas->NM_ajax_info['errList']['geral_rechazar_horas'][] = $msg;
                return;
            }
            rechazar_horas_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function rechazar_horas_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['rechazar_horas'])  && '' != $_SESSION['scriptcase']['error_icon']['rechazar_horas'])  ? $_SESSION['scriptcase']['error_icon']['rechazar_horas']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['rechazar_horas']) && '' != $_SESSION['scriptcase']['error_close']['rechazar_horas']) ? $_SESSION['scriptcase']['error_close']['rechazar_horas'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['rechazar_horas']) && '' != $_SESSION['scriptcase']['error_span_title']['rechazar_horas']) ? $_SESSION['scriptcase']['error_span_title']['rechazar_horas'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['rechazar_horas']) && '' != $_SESSION['scriptcase']['error_icon_title']['rechazar_horas']) ? $_SESSION['scriptcase']['error_icon_title']['rechazar_horas'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['rechazar_horas']['error_buffer'] .= ob_get_flush();
}
