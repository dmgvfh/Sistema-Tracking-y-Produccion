...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_prod_hitos_contractuales']['parms'])) {echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_prod_hitos_contractuales']['parms']);} ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_prod_hitos_contractuales"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_prod_hitos_contractuales']['where_detal']))
  {
?>  
...
{ 
    ...
    if (x == "dyn_search" || x == "dyn_search_clear")
    {
        document.F2.nmgp_arg_dyn_search.value = y; 
    }
...
function scForm_packMultiSelect_multi() {
	...
	for (var Nm_x = 1; Nm_x < NM_count_mult; Nm_x++) {
		nm_pack('periodos_partidas_asociados' + Nm_x + '_dest', 'periodos_partidas_asociados' + Nm_x);
	}
...
{
    NovaJanela = window.open ("form_prod_hitos_contractuales_help.php?nm_cod_campo=" + campo + "&language=<?php echo $this->Ini->path_lang . $this->Ini->str_lang?>&help_css=<?php echo '../_lib/css/' . $this->Ini->str_schema_all . '_form.css'; ?>", "ScriptCase", "height=100, width=300, resizable, scrollbars");
}
