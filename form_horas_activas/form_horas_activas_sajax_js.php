  ...
  function scAjaxUpdateErrors(sType)
  {
    ...
    if (oResp["errList"])
    {
      for (iFieldErrors = 0; iFieldErrors < oResp["errList"].length; iFieldErrors++)
      {
        ...
        if ("geral_form_horas_activas" == sTestField)
        {
...
//    return document.F1.elements[sFieldName].value.replace(/[+]/g, "__NM_PLUS__");
  ...
  function scMoveMasterDetail(widget)
  {
	...
	function scAjax_displayEmptyForm() {
        ...
        sc_hide_form_horas_activas_form();
	}
...
?>
  ...
  // ---------- Validate hora_
  function do_ajax_form_horas_activas_validate_hora_(iNumLinha)
  {
    var nomeCampo_hora_ = "hora_" + iNumLinha;
    var var_hora_ = scAjaxGetFieldText(nomeCampo_hora_);
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_form_horas_activas_validate_hora_(var_hora_, iNumLinha, var_script_case_init, do_ajax_form_horas_activas_validate_hora__cb);
  } // do_ajax_form_horas_activas_validate_hora_

  function do_ajax_form_horas_activas_validate_hora__cb(sResp)
  {
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_form_horas_activas_validate_hora__cb

  // ---------- Validate hora_db_
  function do_ajax_form_horas_activas_validate_hora_db_(iNumLinha)
  {
    var nomeCampo_hora_db_ = "hora_db_" + iNumLinha;
    var var_hora_db_ = scAjaxGetFieldText(nomeCampo_hora_db_);
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_form_horas_activas_validate_hora_db_(var_hora_db_, iNumLinha, var_script_case_init, do_ajax_form_horas_activas_validate_hora_db__cb);
  } // do_ajax_form_horas_activas_validate_hora_db_

  function do_ajax_form_horas_activas_validate_hora_db__cb(sResp)
  {
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    iLineNumber = scAjaxGetLineNumber();
    if ("" != iLineNumber)
    {
      sFieldValid = "hora_db_" + iLineNumber;
    }
    else
    {
      sFieldValid = "hora_db_";
    }
    scEventControl_onBlur(sFieldValid);
    scAjaxUpdateFieldErrors(sFieldValid, "valid");
    sFieldErrors = scAjaxListFieldErrors(sFieldValid, false);
    if ("" == sFieldErrors)
    {
      var sImgStatus = sc_img_status_ok;
      scAjaxHideErrorDisplay(sFieldValid);
      scErrorLineOff(iLineNumber, "hora_db_");
    }
    else
    {
      var sImgStatus = sc_img_status_err;
      scAjaxShowErrorDisplay(sFieldValid, sFieldErrors);
      scErrorLineOn(iLineNumber, "hora_db_");
    }
    var $oImg = $('#id_sc_status_' + sFieldValid);
    if (0 < $oImg.length)
    {
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_form_horas_activas_validate_hora_db__cb

  var sc_num_ult_line = "";
  var sc_insert_open  = false;

  // ---------- add_new_line
  function do_ajax_form_horas_activas_add_new_line(sc_clone, sc_seq_clone)
  {
    if (sc_insert_open)
    {
        if (sc_clone == 'S' && sc_seq_clone != iAjaxNewLine)
        {
          do_ajax_form_horas_activas_cancel_insert(iAjaxNewLine);
        }
        else
        {
          return;
    ...
    var var_script_case_init = document.F1.script_case_init.value;
    scAjaxProcOn(true);
    x_ajax_form_horas_activas_add_new_line(var_sc_clone, var_sc_seq_clone, var_sc_seq_vert, var_script_case_init, do_ajax_form_horas_activas_add_new_line_cb);
  } // do_ajax_form_horas_activas_add_new_line

  function do_ajax_form_horas_activas_add_new_line_cb(sResp)
  {
    scAjaxProcOff(true);
    if ("{" == sResp.substr(0, 1)) {
        oResp = scAjaxResponse(sResp);
  ...
  } // do_ajax_form_horas_activas_add_new_line_cb

  // ---------- backup_line
  function do_ajax_form_horas_activas_backup_line(iNumLinha)
  {
    var var_hora_db_ = scAjaxGetFieldText("hora_db_" + iNumLinha);
    var var_nmgp_refresh_row = iNumLinha;
    var var_nm_form_submit = document.F1.nm_form_submit.value;
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_form_horas_activas_backup_line(var_hora_db_, var_nmgp_refresh_row, var_nm_form_submit, var_script_case_init, do_ajax_form_horas_activas_backup_line_cb);
  } // do_ajax_form_horas_activas_backup_line

  function do_ajax_form_horas_activas_backup_line_cb(sResp)
  {
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    if (!scAjaxHasError())
  ...
  } // do_ajax_form_horas_activas_backup_line_cb

  function do_ajax_form_horas_activas_cancel_insert(iSeqVert)
  {
    var oTBodyOld = document.getElementById("hidden_bloco_0").tBodies[0];
  ...
  } // do_ajax_form_horas_activas_cancel_insert

  function do_ajax_form_horas_activas_cancel_update(iSeqVert)
  {
    do_ajax_form_horas_activas_backup_line(iSeqVert);
...
?>
  } // do_ajax_form_horas_activas_cancel_update

  function do_ajax_form_horas_activas_restore_buttons()
  {
<?php
...
?>
  } // do_ajax_form_horas_activas_restore_buttons

  // ---------- table_refresh
  function do_ajax_form_horas_activas_table_refresh()
  {
    var var_hora_db_ = document.F2.hora_db_.value;
    ...
    scAjaxProcOn();
    x_ajax_form_horas_activas_table_refresh(var_hora_db_, var_nm_form_submit, var_nmgp_opcao, var_nmgp_ordem, var_nmgp_fast_search, var_nmgp_cond_fast_search, var_nmgp_arg_fast_search, var_nmgp_arg_dyn_search, var_script_case_init, do_ajax_form_horas_activas_table_refresh_cb);
  } //  do_ajax_form_horas_activas_table_refresh

  function do_ajax_form_horas_activas_table_refresh_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    ...
    document.F2.hora_db_.value = scAjaxGetKeyValue("hora_db_");
    ...
    scAjaxAlert(do_ajax_form_horas_activas_table_refresh_cb_after_alert);
  } // do_ajax_form_horas_activas_table_refresh_cb
  function do_ajax_form_horas_activas_table_refresh_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
    ...
    sc_insert_open = false;
  } // do_ajax_form_horas_activas_table_refresh_cb_after_alert
...
function scJs_confirm(message, callbackOk, callbackCancel) {
  ...
  var sc_num_ult_opc  = "";
  var sc_num_ult_tr   = "";
  function do_ajax_form_horas_activas_submit_form(iNumLinha, indexTR)
  {
    if (scEventControl_active(iNumLinha)) {
      setTimeout(function() { do_ajax_form_horas_activas_submit_form(iNumLinha, indexTR); }, 500);
      return;
    }
    sc_num_ult_line = iNumLinha;
    sc_num_ult_tr   = indexTR;
    scAjaxHideMessage();
    var var_hora_ = scAjaxGetFieldText("hora_" + iNumLinha);
    var var_hora_db_ = scAjaxGetFieldText("hora_db_" + iNumLinha);
    var var_nm_form_submit = document.F1.nm_form_submit.value;
...
?>
    x_ajax_form_horas_activas_submit_form(var_hora_, var_hora_db_, var_nmgp_refresh_row, var_nm_form_submit, var_nmgp_url_saida, var_nmgp_opcao, var_nmgp_ancora, var_nmgp_num_form, var_nmgp_parms, var_script_case_init, var_csrf_token, do_ajax_form_horas_activas_submit_form_cb);
  } // do_ajax_form_horas_activas_submit_form

  function do_ajax_form_horas_activas_submit_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
...
?>
      ...
      scAjaxHideErrorDisplay("hora_db_" + sc_num_ult_line);
<?php
if (isset($this->Embutida_form) && $this->Embutida_form) {
?>
      scAjaxSetReadonly();
...
?>
    ...
    scAjaxSetLabel(true);
    scAjaxSetMaster();
    scAjaxAlert(do_ajax_form_horas_activas_submit_form_cb_after_alert);
  } // do_ajax_form_horas_activas_submit_form_cb
  function do_ajax_form_horas_activas_submit_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
    scAjaxRedir();
  ...
  } // do_ajax_form_horas_activas_submit_form_cb_after_alert

  var scStatusDetail = {};

  function do_ajax_form_horas_activas_navigate_form()
  {
    perform_ajax_form_horas_activas_navigate_form();
  }

  function perform_ajax_form_horas_activas_navigate_form()
  {
    if (scRefreshTable())
    {
      return;
    ...
    if (sc_insert_open)
    {
        do_ajax_form_horas_activas_cancel_insert(sc_form_horas_activas_get_last_line_number());
    }
    nm_uncheck_delete();
    scAjaxHideMessage();
    scAjaxHideErrorDisplay("table");
    for (iNavForm = 1; iNavForm < <?php echo $this->sc_max_reg; ?> + 1; iNavForm++)
    {
      ...
      scAjaxHideErrorDisplay("hora_db_" + iNavForm);
    }
    var var_hora_db_ = document.F2.hora_db_.value;
    var var_nm_form_submit = document.F2.nm_form_submit.value;
    var var_nmgp_opcao = document.F2.nmgp_opcao.value;
    ...
    var var_script_case_init = document.F2.script_case_init.value;
    scAjaxProcOn();
    x_ajax_form_horas_activas_navigate_form(var_hora_db_, var_nm_form_submit, var_nmgp_opcao, var_nmgp_ordem, var_nmgp_arg_dyn_search, var_script_case_init, do_ajax_form_horas_activas_navigate_form_cb);
  } // do_ajax_form_horas_activas_navigate_form

  var scMasterDetailParentIframe = "<?php echo $_SESSION['sc_session'][$this->Ini->sc_page]['form_horas_activas']['dashboard_info']['parent_widget'] ?>";
...
?>
  function do_ajax_form_horas_activas_navigate_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    ...
    scAjaxSetFields(false);
    scAjaxSetVariables();
    document.F2.hora_db_.value = scAjaxGetKeyValue("hora_db_" + var_last_index);
    ...
    scErrorLineReset();
    $('.sc-js-ui-statusimg').css('display', 'none');
    scAjaxAlert(do_ajax_form_horas_activas_navigate_form_cb_after_alert);
  } // do_ajax_form_horas_activas_navigate_form_cb
  function do_ajax_form_horas_activas_navigate_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
<?php
...
?>
  ...
  } // do_ajax_form_horas_activas_navigate_form_cb_after_alert

    function sc_form_horas_activas_get_last_line_number()
    {
    ...
    } //sc_form_horas_activas_get_last_line_number

  function sc_hide_form_horas_activas_form()
  {
...
?>
  function ajax_create_tables(iNumLinha)
  {
    ...
    iTotCampos++;
    ajax_field_list[iTotCampos] = "hora_db_" + iNumLinha;
    iTotCampos++;
    ajax_error_list["hora_" + iNumLinha] = {"label": "Hora Desplegable<?php echo $sLineInfo; ?>", "valid": new Array(), "onblur": new Array(), "onchange": new Array(), "onclick": new Array(), "onfocus": new Array(), "timeout": 0};
    ajax_error_list["hora_db_" + iNumLinha] = {"label": "Hora Base Datos<?php echo $sLineInfo; ?>", "valid": new Array(), "onblur": new Array(), "onchange": new Array(), "onclick": new Array(), "onfocus": new Array(), "timeout": 0};
    ajax_field_mult["hora_"][iNumLinha] = "hora_" + iNumLinha;
    ajax_field_mult["hora_db_"][iNumLinha] = "hora_db_" + iNumLinha;
    ajax_field_id["hora_" + iNumLinha] = new Array("hidden_field_label_hora_", "hidden_field_data_hora_" + iNumLinha);
    ajax_field_id["hora_db_" + iNumLinha] = new Array("hidden_field_label_hora_db_", "hidden_field_data_hora_db_" + iNumLinha);
    ajax_error_count["hora_" + iNumLinha] = "off";
    ajax_error_count["hora_db_" + iNumLinha] = "off";
<?php
if (!$this->Grid_editavel)
{
?>
    ajax_read_only["hora_" + iNumLinha] = "off";
    ajax_read_only["hora_db_" + iNumLinha] = "off";
<?php
}
else
{
?>
    ...
    ajax_read_only["hora_db_" + iNumLinha] = "on";
<?php
}
?>
  }
  function ajax_destroy_tables(iNumLinha)
  {
    ...
    ajax_error_list["hora_db_" + iNumLinha] = null;
  }
  ...
  function scRemoveErrors()
  {
    for (iNumLinha = 1; iNumLinha < Lim_linhas; iNumLinha++)
    {
      ...
      ajax_error_list["hora_db_" + iNumLinha]["valid"] = new Array();
      ajax_error_list["hora_db_" + iNumLinha]["onblur"] = new Array();
      ajax_error_list["hora_db_" + iNumLinha]["onchange"] = new Array();
      ajax_error_list["hora_db_" + iNumLinha]["onclick"] = new Array();
      ajax_error_list["hora_db_" + iNumLinha]["onfocus"] = new Array();
    }
  }
...
<?php
  $NM_contr_readonly = (isset($this->nmgp_cmp_readonly['hora_db_'])) ? $this->nmgp_cmp_readonly['hora_db_'] : 'off';
?>
    scAjaxFieldRead("hora_db_" + iSeq, "<?php echo $NM_contr_readonly ?>");
  }

  function mdCloseObjects(iSeq)
  {
    scAjaxFieldRead("hora_" + iSeq, "on");
    scAjaxFieldRead("hora_db_" + iSeq, "on");
  }
...
?>
  ...
  function scErrorLineOn(iRow, sIdError)
  {
    ...
    if (bErrorRow || ("on" == ajax_error_count["hora_" + iRow] || "on" == ajax_error_count["hora_db_" + iRow]))
    {
      $("#hidden_field_data_sc_seq" + iRow).addClass("scFormErrorLine");
      $("#hidden_field_data_sc_actions" + iRow).addClass("scFormErrorLine");
      $("#hidden_field_data_hora_" + iRow).addClass("scFormErrorLine");
      $("#hidden_field_data_hora_db_" + iRow).addClass("scFormErrorLine");
    }
  }
  ...
  function scErrorLineOff(iRow, sIdError)
  {
    ...
    if (bErrorRow || ("off" == ajax_error_count["hora_" + iRow] && "off" == ajax_error_count["hora_db_" + iRow]))
    {
      if (bErrorRow)
      {
        ajax_error_count["hora_" + iRow] = "off";
        ajax_error_count["hora_db_" + iRow] = "off";
      }
      ...
      $("#hidden_field_data_hora_" + iRow).removeClass("scFormErrorLine");
      $("#hidden_field_data_hora_db_" + iRow).removeClass("scFormErrorLine");
    }
  }
