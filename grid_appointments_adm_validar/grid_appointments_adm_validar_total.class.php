...
class grid_appointments_adm_validar_total
{
   var $Db;
   var $Erro;
   var $Ini;
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['grid_appointments_adm_validar']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['grid_appointments_adm_validar']['campos_busca']))
      { 
   ...
   {
      global $nm_lang, $accion_revertir, $accion_aprobar, $accion_rechazar;
      $this->nm_data = new nm_data("es");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_adm_validar']['sql_tot_res']);
      ...
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_adm_validar']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('appointment_start_date' => "appointment_start_date",'id_usuario' => "id_usuario",'current_status_id' => "current_status_id",'id_proyecto' => "id_proyecto",'id_actividad' => "id_actividad");
      $cmps_quebra_atual = array();
      foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_adm_validar']['SC_Gb_Free_cmp'] as $cmp => $resto)
      {
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_adm_validar']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_adm_validar']['arr_total'][$cmp_gb] = $this->$Arr_tot_name;
      }
   }
   ...
   function Res_Totaliza_sc_free_group_by($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def, $res_export)
   {
      ...
      $cmp_sql_tp_num = array('appointments_id' => 'N','current_status_id' => 'N','id_proyecto' => 'N','id_usuario' => 'N','id_actividad' => 'N','intervalo' => 'N','intervalo_seg' => 'N');
     ...
     $comando .= " ON " . $join;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_adm_validar']['sql_tot_res']))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_adm_validar']['sql_tot_res'] = str_replace("#@#cmps_quebras#@#", "", $comando);
      }
      $comando  = str_replace("#@#cmps_quebras#@#", "," . $cmps_quebras, $comando);
      $comando .= " group by " . $cmps_quebras . " order by " .  $cmps_quebras;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_adm_validar']['Res_search_metric_use']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_adm_validar']['Res_search_metric_use']))
      {
          $comando = $cmd_simp;
          $cmps_S  = "";
      ...
      $format_dimensions['appointment_start_date']['reg'] = "S";
      $format_dimensions['appointment_start_date']['msk'] = "";
      $format_dimensions['id_usuario']['reg'] = "S";
      $format_dimensions['id_usuario']['msk'] = "";
      $format_dimensions['current_status_id']['reg'] = "S";
      $format_dimensions['current_status_id']['msk'] = "";
      ...
      while (!$rt->EOF)
      {
          ...
          foreach ($Cmp_dim as $Cada_dim => $Ind_sql)
          {
              ...
              if ($Cada_dim == "current_status_id") {
                  $this->Lookup->lookup_sc_free_group_by_current_status_id($$Cada_dim,  $current_status_id);
              }
   ...
   function Calc_resumo_status($destino_resumo, $res_export=false)
   {
      ...
      $this->sc_where_atual  = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_adm_validar']['where_pesq'];
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_adm_validar']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('current_status_id' => "current_status_id");
      $cmps_quebra_atual = array("current_status_id");
      $ult_cmp_quebra_atual = $cmps_quebra_atual[(count($cmps_quebra_atual) - 1)];
      $arr_tots = "";
      $join     = "";
      $group    = "";
     ...
     $comando .= " ON " . $join;
      ...
      while (!$rt->EOF)
      {
          ...
          foreach ($Cmp_dim as $Cada_dim => $Ind_sql)
          {
              ...
              $$SC_orig = $SC_prep['orig'];
              if ($Cada_dim == "current_status_id") {
                  $this->Lookup->lookup_status_current_status_id($$Cada_dim,  $current_status_id);
              }
              if (null === $$Cada_dim)
              {
                  $$Cada_dim = '';
   ...
   function quebra_geral_sc_free_group_by($res_limit=false, $res_export=false)
   {
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_adm_validar']['contr_total_geral'] == "OK") 
      { 
          return; 
      } 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_adm_validar']['tot_geral'] = array() ;  
      $nm_comando = "select count(*), sum(intervalo) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_adm_validar']['where_pesq']; 
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_adm_validar']['tot_geral'][0] = "Total Horas Usuario"; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_adm_validar']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $rt->fields[1] = (string)$rt->fields[1]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_adm_validar']['tot_geral'][2] = $rt->fields[1]; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_adm_validar']['contr_total_geral'] = "OK";
   } 
   ...
   function quebra_current_status_id_sc_free_group_by($current_status_id, $Where_qb, $Cmp_Name) 
   {
      ...
      $tot_current_status_id[0] = NM_encode_input(sc_strip_script($current_status_id)) ; 
      $tot_current_status_id[1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $tot_current_status_id[2] = (string)$rt->fields[1]; 
      $rt->Close(); 
      $$Var_name_gb = $tot_current_status_id;
   } 
   ...
   {
      global $tot_current_status_id, $id_proyecto, $id_usuario, $id_actividad, $current_status_id;
      $tot_current_status_id = array() ;  
      $nm_comando = "select count(*), sum(intervalo) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_adm_validar']['where_pesq']; 
      if (empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_adm_validar']['where_pesq'])) 
      { 
         $nm_comando .= " where current_status_id" . $arg_sum_current_status_id ; 
      } 
      else 
      { 
         $nm_comando .= " and current_status_id" . $arg_sum_current_status_id ; 
      } 
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando;
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = '';
      if (!$rt = $this->Db->Execute($nm_comando)) 
