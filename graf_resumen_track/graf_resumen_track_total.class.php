...
class graf_resumen_track_total
{
   var $Db;
   var $Erro;
   var $Ini;
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['graf_resumen_track']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['graf_resumen_track']['campos_busca']))
      { 
   ...
   function Calc_resumo_sc_free_group_by($destino_resumo, $res_export=false)
   {
      global $nm_lang;
      $this->nm_data = new nm_data("es");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['graf_resumen_track']['sql_tot_res']);
      ...
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['graf_resumen_track']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('appointments_appointment_start_date' => "appointments.appointment_start_date",'appointment_status_appointment_status_descr' => "appointment_status.appointment_status_descr",'sec_users_name' => "sec_users.name",'proyectos_nombre_proyecto' => "proyectos.nombre_proyecto",'tipo_actividades_nombre_actividad' => "tipo_actividades.nombre_actividad");
      $cmps_quebra_atual = array();
      foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['graf_resumen_track']['SC_Gb_Free_cmp'] as $cmp => $resto)
      {
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['graf_resumen_track']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $_SESSION['sc_session'][$this->Ini->sc_page]['graf_resumen_track']['arr_total'][$cmp_gb] = $this->$Arr_tot_name;
      }
   }
   ...
   function Res_Totaliza_sc_free_group_by($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def, $res_export)
   {
      ...
      $cmp_sql_tp_num = array('appointments_appointments_id' => 'N','appointments_intervalo' => 'N');
     ...
     $comando .= " ON " . $join;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['graf_resumen_track']['sql_tot_res']))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['graf_resumen_track']['sql_tot_res'] = str_replace("#@#cmps_quebras#@#", "", $comando);
      }
      $comando  = str_replace("#@#cmps_quebras#@#", "," . $cmps_quebras, $comando);
      $comando .= " group by " . $cmps_quebras . " order by " .  $cmps_quebras;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['graf_resumen_track']['Res_search_metric_use']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['graf_resumen_track']['Res_search_metric_use']))
      {
          $comando = $cmd_simp;
          $cmps_S  = "";
      ...
      $format_dimensions['appointment_status_appointment_status_descr']['reg'] = "S";
      $format_dimensions['appointment_status_appointment_status_descr']['msk'] = "";
      $format_dimensions['sec_users_name']['reg'] = "S";
      $format_dimensions['sec_users_name']['msk'] = "";
      $format_dimensions['proyectos_nombre_proyecto']['reg'] = "S";
      $format_dimensions['proyectos_nombre_proyecto']['msk'] = "";
      $format_dimensions['tipo_actividades_nombre_actividad']['reg'] = "S";
      $format_dimensions['tipo_actividades_nombre_actividad']['msk'] = "";
      while (!$rt->EOF)
      {
          $sql_where = "";
          foreach ($Cmp_dim as $Cada_dim => $Ind_sql)
          {
              ...
              if ($Cada_dim == "appointment_status_appointment_status_descr") {
              }
              if ($Cada_dim == "sec_users_name") {
              }
              if ($Cada_dim == "proyectos_nombre_proyecto") {
              }
              if ($Cada_dim == "tipo_actividades_nombre_actividad") {
              }
              if (null === $$Cada_dim)
              {
                  $$Cada_dim = '';
   ...
   {
      global $nada, $nm_lang ;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['graf_resumen_track']['contr_total_geral'] == "OK") 
      { 
          return; 
      } 
      $_SESSION['sc_session'][$this->Ini->sc_page]['graf_resumen_track']['tot_geral'] = array() ;  
      $nm_comando = "select count(*), sum(appointments.intervalo) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['graf_resumen_track']['where_pesq']; 
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['graf_resumen_track']['tot_geral'][0] = "" . $this->Ini->Nm_lang['lang_msgs_totl'] . ""; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['graf_resumen_track']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $rt->fields[1] = (string)$rt->fields[1]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['graf_resumen_track']['tot_geral'][2] = $rt->fields[1]; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['graf_resumen_track']['contr_total_geral'] = "OK";
   } 
   ...
   function quebra_appointments_appointment_start_date_sc_free_group_by($appointments_appointment_start_date, $Where_qb, $Cmp_Name) 
   {
      ...
      $nm_comando = "select count(*), sum(appointments.intervalo) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['graf_resumen_track']['where_pesq']; 
      if (empty($_SESSION['sc_session'][$this->Ini->sc_page]['graf_resumen_track']['where_pesq'])) 
      { 
   ...
   //-----  appointment_status_appointment_status_descr
   function quebra_appointment_status_appointment_status_descr_sc_free_group_by($appointment_status_appointment_status_descr, $Where_qb, $Cmp_Name) 
   {
      $Var_name_gb = "SC_tot_" . $Cmp_Name;
      global $$Var_name_gb;
      $tot_appointment_status_appointment_status_descr = array() ;  
      $nm_comando = "select count(*), sum(appointments.intervalo) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['graf_resumen_track']['where_pesq']; 
      if (empty($_SESSION['sc_session'][$this->Ini->sc_page]['graf_resumen_track']['where_pesq'])) 
      { 
         $nm_comando .= " where " . $Where_qb ; 
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['graf_resumen_track']['opcao'] == "pdf" && isset($_SESSION['nm_session']['sys_wkhtmltopdf_show_html_content']) && $_SESSION['nm_session']['sys_wkhtmltopdf_show_html_content'] == 'Y') {
          $tot_appointment_status_appointment_status_descr[0] = NM_encode_input(sc_strip_script($appointment_status_appointment_status_descr));
      }
      else {
          $tot_appointment_status_appointment_status_descr[0] = sc_strip_script($appointment_status_appointment_status_descr) ; 
      }
      $tot_appointment_status_appointment_status_descr[1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $tot_appointment_status_appointment_status_descr[2] = (string)$rt->fields[1]; 
      $rt->Close(); 
      $$Var_name_gb = $tot_appointment_status_appointment_status_descr;
   } 

   //-----  sec_users_name
   function quebra_sec_users_name_sc_free_group_by($sec_users_name, $Where_qb, $Cmp_Name) 
   {
      $Var_name_gb = "SC_tot_" . $Cmp_Name;
      global $$Var_name_gb;
      $tot_sec_users_name = array() ;  
      $nm_comando = "select count(*), sum(appointments.intervalo) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['graf_resumen_track']['where_pesq']; 
      if (empty($_SESSION['sc_session'][$this->Ini->sc_page]['graf_resumen_track']['where_pesq'])) 
      { 
         $nm_comando .= " where " . $Where_qb ; 
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['graf_resumen_track']['opcao'] == "pdf" && isset($_SESSION['nm_session']['sys_wkhtmltopdf_show_html_content']) && $_SESSION['nm_session']['sys_wkhtmltopdf_show_html_content'] == 'Y') {
          $tot_sec_users_name[0] = NM_encode_input(sc_strip_script($sec_users_name));
      }
      else {
          $tot_sec_users_name[0] = sc_strip_script($sec_users_name) ; 
      }
      $tot_sec_users_name[1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $tot_sec_users_name[2] = (string)$rt->fields[1]; 
      $rt->Close(); 
      $$Var_name_gb = $tot_sec_users_name;
   } 

   //-----  proyectos_nombre_proyecto
   function quebra_proyectos_nombre_proyecto_sc_free_group_by($proyectos_nombre_proyecto, $Where_qb, $Cmp_Name) 
   {
      $Var_name_gb = "SC_tot_" . $Cmp_Name;
      global $$Var_name_gb;
      $tot_proyectos_nombre_proyecto = array() ;  
      $nm_comando = "select count(*), sum(appointments.intervalo) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['graf_resumen_track']['where_pesq']; 
      if (empty($_SESSION['sc_session'][$this->Ini->sc_page]['graf_resumen_track']['where_pesq'])) 
      { 
         $nm_comando .= " where " . $Where_qb ; 
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['graf_resumen_track']['opcao'] == "pdf" && isset($_SESSION['nm_session']['sys_wkhtmltopdf_show_html_content']) && $_SESSION['nm_session']['sys_wkhtmltopdf_show_html_content'] == 'Y') {
          $tot_proyectos_nombre_proyecto[0] = NM_encode_input(sc_strip_script($proyectos_nombre_proyecto));
      }
      else {
          $tot_proyectos_nombre_proyecto[0] = sc_strip_script($proyectos_nombre_proyecto) ; 
      }
      $tot_proyectos_nombre_proyecto[1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $tot_proyectos_nombre_proyecto[2] = (string)$rt->fields[1]; 
      $rt->Close(); 
      $$Var_name_gb = $tot_proyectos_nombre_proyecto;
   } 

   //-----  tipo_actividades_nombre_actividad
   function quebra_tipo_actividades_nombre_actividad_sc_free_group_by($tipo_actividades_nombre_actividad, $Where_qb, $Cmp_Name) 
   {
      $Var_name_gb = "SC_tot_" . $Cmp_Name;
      global $$Var_name_gb;
      $tot_tipo_actividades_nombre_actividad = array() ;  
      $nm_comando = "select count(*), sum(appointments.intervalo) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['graf_resumen_track']['where_pesq']; 
      if (empty($_SESSION['sc_session'][$this->Ini->sc_page]['graf_resumen_track']['where_pesq'])) 
      { 
         $nm_comando .= " where " . $Where_qb ; 
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['graf_resumen_track']['opcao'] == "pdf" && isset($_SESSION['nm_session']['sys_wkhtmltopdf_show_html_content']) && $_SESSION['nm_session']['sys_wkhtmltopdf_show_html_content'] == 'Y') {
          $tot_tipo_actividades_nombre_actividad[0] = NM_encode_input(sc_strip_script($tipo_actividades_nombre_actividad));
      }
      else {
          $tot_tipo_actividades_nombre_actividad[0] = sc_strip_script($tipo_actividades_nombre_actividad) ; 
      }
      $tot_tipo_actividades_nombre_actividad[1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $tot_tipo_actividades_nombre_actividad[2] = (string)$rt->fields[1]; 
      $rt->Close(); 
      $$Var_name_gb = $tot_tipo_actividades_nombre_actividad;
   } 
