...
class grid_appointments_staff_resumo
{
   var $Db;
   var $Erro;
   var $Ini;
   ...
   function initializeButtons()
   {
      ...
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['dashboard_info']['under_dashboard']) && $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['dashboard_info']['under_dashboard'] && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['dashboard_info']['maximized']) {
          $tmpDashboardApp = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['dashboard_info']['dashboard_app'];
          if (isset($_SESSION['scriptcase']['dashboard_toolbar'][$tmpDashboardApp]['grid_appointments_staff'])) {
              $tmpDashboardButtons = $_SESSION['scriptcase']['dashboard_toolbar'][$tmpDashboardApp]['grid_appointments_staff'];
   ...
   if ($this->Ini->Embutida_iframe) {
       foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['sub_cons_iframe_btns'] as $BTN => $BTN_opc) {
           $this->nmgp_botoes[$BTN] = $BTN_opc;
       }
   }
      if (isset($_SESSION['scriptcase']['sc_apl_conf']['grid_appointments_staff']['btn_display']) && !empty($_SESSION['scriptcase']['sc_apl_conf']['grid_appointments_staff']['btn_display']))
      {
          foreach ($_SESSION['scriptcase']['sc_apl_conf']['grid_appointments_staff']['btn_display'] as $NM_cada_btn => $NM_cada_opc)
          {
   ...
   { 
	...
	function generateChartImages() {
		...
		$chartList  = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['pivot_charts'];
		$chartFiles = array();

		$_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['phantomjs_export_process'] = true;
		$_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['export_progress_step']     = 'image';
		$_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['export_progress_count']    = 0;
		$_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['export_progress_total']    = 0;
		...
		foreach ($chartList as $chartKey => $chartData) {
			...
			$_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['export_progress_total']++;
		}
		...
		foreach ($chartList as $chartKey => $chartData) {
			...
			$_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['phantomjs_export_file'] = '';
			$_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['export_progress_count']++;

			$this->writeProgressFile();

			$this->Graf->generateChartImage($chartKey);

			if ('' != $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['phantomjs_export_file']) {
				$chartFiles[] = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['phantomjs_export_file'];
			}
		}

		$_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['phantomjs_export_process'] = false;
	...
	function zipChartImages($password = '') {
		...
		$_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['export_progress_step'] = 'zip';

		$this->writeProgressFile();

		$zipFile = $this->zipFileList($chartImages, $password);

		$_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['export_progress_step'] = 'end';
	...
	function zipFileList($fileList, $password = '') {
		$tempDir     = $this->Ini->root . $_SESSION['scriptcase']['grid_appointments_staff']['glo_nm_path_imag_temp'] . '/';
	...
	function writeProgressFile() {
		if ('image' == $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['export_progress_step']) {
			$progress = floor($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['export_progress_count'] * 100 / ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['export_progress_total'] + 1));
			$content  = $this->Ini->Nm_lang['lang_pdff_pcht'] . ": {$_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['export_progress_count']}/{$_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['export_progress_total']}###$progress";

			$content  = $_SESSION['scriptcase']['charset'] != 'UTF-8' ? sc_convert_encoding($content, 'UTF-8', $_SESSION['scriptcase']['charset']) : $content;
		}
		elseif ('zip' == $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['export_progress_step']) {
			$progress = floor($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['export_progress_count'] * 100 / ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['export_progress_total'] + 1));
			$content  = $this->Ini->Nm_lang['lang_chrt_zip_img'] . "###$progress";

			$content  = $_SESSION['scriptcase']['charset'] != 'UTF-8' ? sc_convert_encoding($content, 'UTF-8', $_SESSION['scriptcase']['charset']) : $content;
		}
		elseif ('end' == $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['export_progress_step']) {
			$content = "ok###100";
		}
		else {
			$content = "init###0";
		...
		$f = @fopen("{$this->Ini->root}{$_SESSION['scriptcase']['grid_appointments_staff']['glo_nm_path_imag_temp']}/{$_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['export_progress_file']}", 'w');
		fwrite($f, $content);
		fclose($f);
	}
   ...
   function monta_resumo($b_export = false)
   {
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['orig_pesq'] = "resumo";
...
if (!isset($_SESSION['glo_staff_id'])) {$_SESSION['glo_staff_id'] = "";}
...
if ($this->sc_where_atual =="")
	{
$sql_where = "WHERE id_usuario = $this->sc_temp_glo_staff_id and  YEAR(appointment_start_date) = YEAR(CURRENT_DATE()) 
...
$this->nm_where_dinamico = $sql_where;
	...
	{
$sql_where = " and id_usuario = $this->sc_temp_glo_staff_id";
$this->nm_where_dinamico = $sql_where;
	}
if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['where_dinamico']) || $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['where_dinamico'] != $this->nm_where_dinamico) {
    $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['where_dinamico'] = $this->nm_where_dinamico;
    unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['inicio']);
}
...
$_SESSION['scriptcase']['grid_appointments_staff']['contr_erro'] = 'off'; 
       $this->SC_Buf_onInit = ob_get_clean();; 
         ...
         {   
             $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['where_pesq'] .= $this->nm_where_dinamico;
         }
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['opcao'] != "pdf")
      {
          unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['where_resumo']);
      }
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['proc_res'] = true;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['res_hrz']))
      { 
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['res_hrz'] = $this->NM_totaliz_hrz;
      } 
      $this->NM_totaliz_hrz = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['res_hrz'];
      $dir_raiz          = strrpos($_SERVER['PHP_SELF'],"/") ;  
      $dir_raiz          = substr($_SERVER['PHP_SELF'], 0, $dir_raiz + 1) ;  
      $this->nm_location = $this->Ini->sc_protocolo . $this->Ini->server . $dir_raiz; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['array_graf_pdf'] = array();
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['opcao'] != "pdf")
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['where_resumo'] = "";
      }
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['contr_array_resumo'] == "OK" && $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['contr_total_geral'] == "OK")
      {
          $this->array_total_appointment_start_date = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['arr_total']['appointment_start_date'];
          $this->array_total_current_status_id = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['arr_total']['current_status_id'];
      }
      else
      {
          $this->array_total_appointment_start_date = array();
      ...
      $this->array_total_geral[0] = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['tot_geral'][2];
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['tot_geral'][1]) || $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['tot_geral'][1] == 0)
      {
      ...
      if (($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['opcao'] == 'print' || $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['opcao'] == 'pdf') && strpos(" " . $this->Ini->SC_module_export, "resume") === false)
      { }
      else
      {
          $this->drawMatrix();
      ...
      $this->resumo_final();
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['contr_label_graf'] = array();
      if (isset($this->nmgp_label_quebras) && !empty($this->nmgp_label_quebras))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['contr_label_graf'] = $this->nmgp_label_quebras;
      }
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['arr_total']['appointment_start_date'] = $this->array_total_appointment_start_date;
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['arr_total']['current_status_id'] = $this->array_total_current_status_id;
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['contr_array_resumo'] = "OK";
   }
   ...
   function completeMatrix()
   {
       ...
       //-----
       if ($this->NM_export && isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['json_use_label']) && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['json_use_label'])
       {
       ...
       {
       $Lab_appointment_start_date = "Fecha Actividad";
       $Lab_appointment_start_date =  sprintf("" . $this->Ini->Nm_lang['lang_othr_cons_title_YYYYMM'] . "", $Lab_appointment_start_date);
       $Lab_current_status_id = "" . $this->Ini->Nm_lang['lang_appointments_fld_current_status_id'] . "";
       }
       $this->comp_field = array(
           $Lab_appointment_start_date,
           $Lab_current_status_id,
       );

       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['labels']['appointment_start_date']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['labels']['appointment_start_date'] = "Fecha Actividad"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['labels']['appointment_start_time']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['labels']['appointment_start_time'] = "Hora Inicio"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['labels']['appointment_end_time']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['labels']['appointment_end_time'] = "Hora Término"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['labels']['intervalo']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['labels']['intervalo'] = "Duración (horas)"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['labels']['id_proyecto']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['labels']['id_proyecto'] = "Proyecto"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['labels']['id_actividad']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['labels']['id_actividad'] = "Tipo Actividad"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['labels']['nombre']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['labels']['nombre'] = "Nombre Actividad"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['labels']['observaciones']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['labels']['observaciones'] = "Observaciones"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['labels']['current_status_id']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['labels']['current_status_id'] = "" . $this->Ini->Nm_lang['lang_appointments_fld_current_status_id'] . ""; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['labels']['historia']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['labels']['historia'] = "Historia"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['labels']['appointments_id']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['labels']['appointments_id'] = "" . $this->Ini->Nm_lang['lang_appointments_fld_appointments_id'] . ""; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['labels']['appointment_end_date']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['labels']['appointment_end_date'] = "Fecha Término"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['labels']['appointment_recurrent']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['labels']['appointment_recurrent'] = "" . $this->Ini->Nm_lang['lang_appointments_fld_appointment_recurrent'] . ""; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['labels']['appointment_period']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['labels']['appointment_period'] = "" . $this->Ini->Nm_lang['lang_appointments_fld_appointment_period'] . ""; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['labels']['id_usuario']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['labels']['id_usuario'] = "Id Usuario"; 
       }
       //-----
       $this->comp_field_nm = array(
       ...
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['summarizing_fields_order']['status']))
       {
           foreach ($this->comp_sum as $i_sum => $l_sum)
           {
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['summarizing_fields_order']['status'][] = $i_sum;
           }
       }
       else
       {
           $this->comp_sum_order = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['summarizing_fields_order']['status'];
       }
       ...
       $this->comp_sum_display = array(
           ...
           foreach ($this->comp_sum as $i_sum => $l_sum)
           {
               if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['summarizing_fields_display']['status'][$i_sum]))
               {
                   $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['summarizing_fields_display']['status'][$i_sum] = array('label' => $l_sum, 'display' => $this->comp_sum_display[$i_sum]);
               }
               else
               {
                   if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['summarizing_fields_display']['status'][$i_sum]['label']))
                   {
                       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['summarizing_fields_display']['status'][$i_sum]['label'] = $l_sum;
                   }
                   if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['summarizing_fields_display']['status'][$i_sum]['display']))
                   {
                       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['summarizing_fields_display']['status'][$i_sum]['display'] = $this->comp_sum_display[$i_sum];
                   }
               }
           }
           foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['summarizing_fields_display']['status'] as $i_sum => $d_sum)
           {
       ...
       //-----
       foreach ($this->array_total_appointment_start_date as $i_appointment_start_date => $d_appointment_start_date) {
           if (!isset($i_appointment_start_date, $this->comp_label[0]) || !in_array($i_appointment_start_date, $this->comp_label[0], true)) {
               $this->comp_index[0][ $d_appointment_start_date[3] ] = $d_appointment_start_date[2];
               $this->comp_label[0][ $d_appointment_start_date[3] ] = $d_appointment_start_date[2];
           }
           foreach ($this->array_total_current_status_id[$i_appointment_start_date] as $i_current_status_id => $d_current_status_id) {
               if (!isset($i_current_status_id, $this->comp_label[1][ $d_appointment_start_date[3] ]) || !in_array($i_current_status_id, $this->comp_label[1][ $d_appointment_start_date[3] ], true)) {
                   $this->comp_index[1][ $d_current_status_id[3] ] = $d_current_status_id[2];
                   $this->comp_label[1][ $d_appointment_start_date[3] ][ $d_current_status_id[3] ] = $d_current_status_id[2];
               }
           }
       }
       ...
       //-----
       $this->comp_links_fl = array(
           array('name' => 'appointment_start_date', 'prot' => '@aspass@'),
           array('name' => 'current_status_id', 'prot' => ''),
       );
       ...
       //-----
       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['pivot_group_by'] = $this->comp_field;
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['pivot_x_axys']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['pivot_x_axys'] = $this->comp_x_axys;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['pivot_y_axys']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['pivot_y_axys'] = $this->comp_y_axys;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['pivot_fill']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['pivot_fill'] = $this->comp_fill;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['pivot_order']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['pivot_order'] = $this->comp_order;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['pivot_order_start']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['pivot_order_start'] = $this->comp_order_start;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['pivot_tabular']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['pivot_tabular'] = $this->comp_tabular;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['pivot_tabular_hover']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['pivot_tabular_hover'] = $this->comp_tabular && $this->comp_tab_hover;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['pivot_tabular_seq']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['pivot_tabular_seq'] = $this->comp_tabular && $this->comp_tab_seq;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['pivot_tabular_label']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['pivot_tabular_label'] = $this->comp_tabular && $this->comp_tab_label;
       }

       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['pivot_order_col']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['pivot_order_col'] = 0;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['pivot_order_level']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['pivot_order_level'] = 0;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['pivot_order_sort']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['pivot_order_sort'] = '';
       }
       if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['ajax_nav'] && isset($_POST['parm']))
       { 
       ...
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['pivot_order_sort'] = $_POST['sort_ord'];
           if ('' == $_POST['sort_ord'])
           {
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['pivot_order_col']  = 0;
           }
           ...
           {
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['pivot_order_col']  = $_POST['sort_col'];
           }
       }
       if (isset($_POST['change_sort']) && 'NEW' == $_POST['change_sort']) {
           for ($i = 0; $i < sizeof($this->comp_label); $i++) {
               if ($i == $_POST['sort_col']) {
                   $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['pivot_order_start'][$i] = $_POST['sort_ord'];
               }
           }
       }

       $this->comp_x_axys      = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['pivot_x_axys'];
       $this->comp_y_axys      = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['pivot_y_axys'];
       $this->comp_fill        = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['pivot_fill'];
       $this->comp_order       = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['pivot_order'];
       $this->comp_order_start = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['pivot_order_start'];
       $this->comp_order_col   = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['pivot_order_col'];
       $this->comp_order_level = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['pivot_order_level'];
       $this->comp_order_sort  = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['pivot_order_sort'];
       $this->comp_tabular     = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['pivot_tabular'];
       $this->comp_tab_hover   = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['pivot_tabular_hover'];
       $this->comp_tab_seq     = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['pivot_tabular_seq'];
       $this->comp_tab_label   = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['pivot_tabular_label'];
     ...
     if (isset($this->comp_label[0])) {
       foreach ($this->comp_label[0] as $i_appointment_start_date => $l_appointment_start_date) {
           if (isset($this->array_total_appointment_start_date[$i_appointment_start_date])) {
               $this->comp_group[$i_appointment_start_date] = array();
           }
           foreach ($this->comp_label[1][$i_appointment_start_date] as $i_current_status_id => $l_current_status_id) {
               if (isset($this->array_total_current_status_id[$i_appointment_start_date][$i_current_status_id])) {
                   $this->comp_group[$i_appointment_start_date][$i_current_status_id] = array();
               }
           }
       }
     }
   ...
   function getCompData($level, $params = array()) {
       if (0 == $level) {
           $return = $this->array_total_appointment_start_date;
       }
       elseif (1 == $level) {
           $return = $this->array_total_current_status_id;
       }
   ...
   function getLabelLink($param, $i_tmp = -1, $bProtect = true)
   {
       ...
       $Md5_Res    = "@SC_par@" . NM_encode_input($this->Ini->sc_page) . "@SC_par@grid_appointments_staff@SC_par@" . md5($Parms_Res);
       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['LigR_Md5'][md5($Parms_Res)] = $Parms_Res;
       return $Md5_Res;
   }
   ...
   function makeTabular(&$a_data)
   {
       ...
       {
           $a_labels = array();
   if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['proc_pdf'] || $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['proc_pdf_vert'])
   {
   ...
   function buildChart()
   {
       ...
       $this->comp_chart_config = array(
           '0|1' => array(
               'title'    => $this->getChartText(strip_tags("Total Horas De Actividades Por Mes")),
               'show_sub' => true,
               'subtitle' => "",
           ...
           '1|1' => array(
               'title'    => $this->getChartText(strip_tags("Total Horas (Suma)")),
               'show_sub' => true,
               'subtitle' => "",
               'field_x'  => 'current_status_id',
               'field_y'  => $this->comp_sum_nm[2],
               'label_x'  => $this->getChartText(strip_tags($Lab_current_status_id)),
               'label_y'  => $this->getChartText(strip_tags("Total Horas (Suma)")),
               'format'   => $this->formatChartValue(1),
           ),
           'T|1' => array(
       ...
       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['pivot_charts']    = $this->comp_chart_data;
       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['pivot_orig']      = $this->comp_chart_data;
       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['comb_table_data'] = array();
       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['comb_table_def']  = array();
       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['anlt_table_data'] = array();
       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['anlt_table_def']  = array();
           ...
           foreach ($a_totals as $i_total => $values)
           {
               ...
               if (isset($this->comp_chart_config[$key_config]))
               {
                   ...
                   foreach ($a_keys as $tmp_key)
                   {
                       ...
                       $xml_links[]  = $this->getFusionLink('sc_grid_appointments_staff_' . session_id() . '_!!!' . implode('_', array_merge(array($group + 1, $i_total), $link_index)) . '!!!.json');
                   }
                   ...
                   $this->comp_chart_data[$key_data]['xml']        = $this->getFusionLink('sc_grid_appointments_staff_' . session_id() . '_!!!' . implode('_', array_merge(array($group, $i_total), $param)) . '!!!.json');
   ...
   function drawMatrix()
   {
       ...
       if ($this->NM_export)
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['arr_export']['label'] = $this->build_labels;
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['arr_export']['data']  = $this->build_data;
           return;
       }
      ...
      if ($this->comp_tab_hover)
      {
       ...
       foreach ($this->build_data as $row_i => $lines)
       {
           ...
           foreach ($lines as $col_i => $columns)
           {
               ...
               {
                   if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['opcao'] != "pdf" && $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['opcao'] != "print" && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['embutida'])
                   {
               ...
               $chart   = (($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['opcao'] != "print" && $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['opcao'] != "pdf" && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['embutida']) && isset($columns['chart']) && '' != $columns['chart'] && isset($this->comp_chart_data[ $columns['chart'] ]))
           ...
           if (!$b_display)
           {
               if ($this->comp_tabular)
               {
                   ...
                   foreach ($this->comp_y_axys as $iYAxysIndex)
                   {
                       ...
                       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['pivot_order_start'][$iYAxysIndex]))
                       {
                           $sInitialOrder   = '';
                           $sInitialDisplay = '; display: none';
                           $sInitialSrc     = '';
                       }
                       elseif ('asc' == $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['pivot_order_start'][$iYAxysIndex])
                       {
...
if($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['proc_pdf'] || $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['proc_pdf_vert']){ 
   ...
   function resumo_init()
   {
      ...
      if ("out" == $this->NM_tipo)
      {
         $this->monta_html_ini();
         $this->monta_cabecalho();
         if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['opcao'] != "pdf" && !$_SESSION['scriptcase']['proc_mobile'])
         {
   ...
   function monta_css()
   {
      global $nm_saida, $nmgp_tipo_pdf, $nmgp_cor_print;
       ...
       if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['embutida'])
       {
          if (($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['opcao'] == "print" && strtoupper($nmgp_cor_print) == "PB") || $nmgp_tipo_pdf == "pb")
           { 
               if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['SC_sub_css_bw']['grid_appointments_staff']))
               {
                   $compl_css = str_replace(".", "_", $_SESSION['sc_session'][$this->Ini->sc_page]['SC_sub_css_bw']['grid_appointments_staff']) . "_";
               } 
           } 
           else 
           { 
               if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['SC_sub_css']['grid_appointments_staff']))
               {
                   $compl_css = str_replace(".", "_", $_SESSION['sc_session'][$this->Ini->sc_page]['SC_sub_css']['grid_appointments_staff']) . "_";
               } 
           }
       }
   ...
   function inicializa_vars()
   {
      ...
      $this->Print_All = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['print_all'];
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['doc_word'] || $this->Ini->sc_export_ajax_img)
      { 
      ...
      $this->Total   = new grid_appointments_staff_total($this->Ini->sc_page);
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['LigR_Md5'] = array();
   }
   ...
   function totaliza()
   {
      ...
      foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['arr_total'] as $cmp_gb => $resto)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $this->$Arr_tot_name = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['arr_total'][$cmp_gb];
      }
      if (is_array($this->array_total_appointment_start_date)) {
          ksort($this->array_total_appointment_start_date);
      }
      if (is_array($this->array_total_current_status_id)) {
          ksort($this->array_total_current_status_id);
      }
      if ("out" == $this->NM_tipo)
      {
         $Gb_geral = "quebra_geral_" . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['SC_Ind_Groupby'];
...
if (!$_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['responsive_chart']['active']) {
$nm_saida->saida("<!DOCTYPE HTML PUBLIC \"-//W3C//DTD HTML 4.01 Transitional//EN\"\r\n");
$nm_saida->saida("            \"http://www.w3.org/TR/1999/REC-html401-19991224/loose.dtd\">\r\n");
}
$_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['dashboard_refresh_after_chart'] = 'resumo';
      $nm_saida->saida("<HTML" . $_SESSION['scriptcase']['reg_conf']['html_dir'] . ">\r\n");
      $nm_saida->saida("<HEAD>\r\n");
      $nm_saida->saida(" <TITLE>Resumen de Horas De  Actividades - Bitácora Personal</TITLE>\r\n");
...
if (!$_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['doc_word'])
{
      ...
      { 
           ...
           $nm_saida->saida(" <script type=\"text/javascript\" src=\"grid_appointments_staff_ajax.js\"></script>\r\n");
           $nm_saida->saida(" <script type=\"text/javascript\">\r\n");
           ...
           $nm_saida->saida(" </script>\r\n");
           $nm_saida->saida(" <script type=\"text/javascript\" src=\"grid_appointments_staff_jquery-3.6.0.min.js\"></script>\r\n");
           $nm_saida->saida(" <script type=\"text/javascript\" src=\"grid_appointments_staff_message.js\"></script>\r\n");
           $nm_saida->saida(" <script type=\"text/javascript\" src=\"../_lib/lib/js/scInput.js\"></script>\r\n");
           $nm_saida->saida(" <script type=\"text/javascript\" src=\"../_lib/lib/js/jquery.scInput.js\"></script>\r\n");
           $nm_saida->saida(" <script type=\"text/javascript\" src=\"../_lib/lib/js/jquery.scInput2.js\"></script>\r\n");
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['num_css']))
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['num_css'] = rand(0, 1000);
      }
      $NM_css = @fopen($this->Ini->root . $this->Ini->path_imag_temp . '/sc_css_grid_appointments_staff_sum_' . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['num_css'] . '.css', 'w');
      if (($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['opcao'] == "print" && strtoupper($nmgp_cor_print) == "PB") || $nmgp_tipo_pdf == "pb")
      {
      ...
      @fclose($NM_css);
     $this->Ini->summary_css = $this->Ini->path_imag_temp . '/sc_css_grid_appointments_staff_sum_' . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['num_css'] . '.css';
     if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['opcao'] == "print")
     {
         $nm_saida->saida("  <style type=\"text/css\">\r\n");
         $NM_css = file($this->Ini->root . $this->Ini->path_imag_temp . '/sc_css_grid_appointments_staff_sum_' . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['num_css'] . '.css');
     ...
     $nm_saida->saida("   <link rel=\"stylesheet\" type=\"text/css\" href=\"" . $this->Ini->path_link . "grid_appointments_staff/grid_appointments_staff_res_" . strtolower($_SESSION['scriptcase']['reg_conf']['css_dir']) . ".css\" />\r\n");
     $nm_saida->saida(" <style type=\"text/css\">\r\n");
...
if ($_SESSION['scriptcase']['proc_mobile'])
{
      ...
      if (!$this->Ini->Export_html_zip && $this->Print_All && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['doc_word']) 
      {
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['opcao'] != "print" && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['embutida'])
      {
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['opcao'] != "pdf" && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['embutida'] && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['doc_word'])
      { 
          $nm_saida->saida("      <STYLE>\r\n");
   ...
   function monta_barra_top_normal()
   {
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['opcao'] != "print" && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['embutida'])
      {
         ...
         $NM_btn = false;
         $NM_Gbtn = false;
         if (is_file("grid_appointments_staff_help.txt") && !$this->grid_emb_form && !$this->NM_res_sem_reg)
         {
            $Arq_WebHelp = file("grid_appointments_staff_help.txt"); 
   ...
   function monta_barra_top_mobile()
   {
      ...
      if (!$this->grid_emb_form && $this->nmgp_botoes['pdf'] == "on" && !$this->NM_res_sem_reg)
      {
         ...
         $Cod_Btn = nmButtonOutput($this->arr_buttons, "bpdf", "", "", "Rpdf_top", "", "", "", "absmiddle", "", "0px", $this->Ini->path_botoes, "", "", "thickbox", "" . $this->Ini->path_link . "grid_appointments_staff/grid_appointments_staff_config_pdf.php?nm_opc=pdf_res&nm_target=0&nm_cor=cor&papel=1&orientacao=1&bookmarks=1&largura=1200&conf_larg=S&conf_fonte=10&grafico=S&sc_ver_93=n&nm_tem_gb=s&nm_res_cons=n&nm_ini_pdf_res=resume,chart&nm_all_modules=grid,resume,chart&password=n&summary_export_columns=N&nm_label_group=S&nm_all_cab=N&nm_all_label=N&pdf_zip=N&nm_orient_grid=2&origem=res&language=es&conf_socor=S&script_case_init=" . $this->Ini->sc_page . "&app_name=grid_appointments_staff&KeepThis=true&TB_iframe=true&modal=true", "group_1", "only_text", "text_right", "", "", "", "", "", "", "");
         $nm_saida->saida("           $Cod_Btn \r\n");
          $nm_saida->saida("            </div>\r\n");
      }
         if ($NM_Gbtn)
   ...
   function monta_html_fim()
   {
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['print_all'])
      {
          $this->grafico_pdf();
      }
      $nm_saida->saida("</TABLE>\r\n");
...
{ 
      ...
      $nm_saida->saida("</TABLE>\r\n");
       ...
       $nm_saida->saida("  tb_show('', 'grid_appointments_staff_config_pivot.php?nome_apl=grid_appointments_staff&sc_page=" . NM_encode_input($this->Ini->sc_page) . "&language=es&TB_iframe=true&modal=true&height=300&width=500', '');\r\n");
       $nm_saida->saida("}\r\n");
   ...
   $nm_saida->saida("                  action=\"grid_appointments_staff_iframe_prt.php\" \r\n");
   ...
   $nm_saida->saida("<SCRIPT language=\"Javascript\">\r\n");
      ...
      $nm_saida->saida("               nm_submit_modal(\"" . $this->Ini->path_link . "grid_appointments_staff/grid_appointments_staff_export_email.php?script_case_init={$this->Ini->sc_page}&path_img={$this->Ini->path_img_global}&path_btn={$this->Ini->path_botoes}&sType=\"+ str_type +\"&sAdd=__E__nmgp_cor_word=\" + cor + \"__E__SC_module_export=\" + SC_module_export + \"__E__nmgp_password=\" + password + \"&KeepThis=true&TB_iframe=true&modal=true\", bol_param);\r\n");
      $nm_saida->saida("       }\r\n");
      $nm_saida->saida("       else\r\n");
      $nm_saida->saida("       {\r\n");
      ...
      $nm_saida->saida("           document.Fexport.nmgp_password.value = password;\r\n");
      $nm_saida->saida("           document.Fexport.SC_module_export.value = SC_module_export;\r\n");
      $nm_saida->saida("               document.Fexport.action = \"grid_appointments_staff_export_ctrl.php\";\r\n");
      $nm_saida->saida("           document.Fexport.submit() ;\r\n");
      $nm_saida->saida("       }\r\n");
      $nm_saida->saida("   }\r\n");
      ...
      $nm_saida->saida("       document.F3.SC_module_export.value = z;\r\n");
      $nm_saida->saida("  }\r\n");
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['grid_appointments_staff_iframe_params'] = array(
      ...
      $Md5_pdf    = "@SC_par@" . NM_encode_input($this->Ini->sc_page) . "@SC_par@grid_appointments_staff@SC_par@" . md5($prep_parm_pdf);
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['Md5_pdf'][md5($prep_parm_pdf)] = $prep_parm_pdf;
      ...
      $nm_saida->saida("          document.Fres_pdf.action = \"grid_appointments_staff_iframe.php\";\r\n");
   ...
   function monta_html_ini_pdf()
   {
      global $nm_saida;
       $tp_quebra = "";
       if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['num_css']) && !empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['num_css']))
       {
           $NM_css = @fopen($this->Ini->root . $this->Ini->path_imag_temp . '/sc_css_grid_appointments_staff_grid_' . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['num_css'] . '.css', 'a');
           $NM_css_file = $this->Ini->root . $this->Ini->path_link . "grid_appointments_staff/grid_appointments_staff_res_" . strtolower($_SESSION['scriptcase']['reg_conf']['css_dir']). ".css";
       ...
       $this->Print_All = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['print_all'];
       if (!$_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['pdf_res'])
       {
           $nm_saida->saida("<TR>\r\n");
       ...
       if ($this->Print_All || $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['print_all'])
       {
       $tp_quebra = "<div style=\"page-break-after: always;\"><span style=\"display: none;\">&nbsp;</span></div>";
       }
...
//--- 
 function grafico_pdf()
 {
   ...
   $this->Graf->Ini    = $this->Ini;
   $this->Graf->Lookup = $this->Lookup;
   if (!empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['pivot_charts']) && (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['skip_charts']) || !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['skip_charts']))
   {
       ...
       $iChartCount = 1;
       $iChartTotal = sizeof($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['pivot_charts']);
 ...
 { 
       ...
       foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['pivot_charts'] as $chart_index => $chart_data)
       {
           if (!$prim_graf)
           {
 ...
 { 
           ...
           if ($this->progress_fp)
           {
               grid_appointments_staff_pdf_progress_call($this->progress_tot . "_#NM#_" . $this->progress_now . "_#NM#_" . $sChartLang . " " . $iChartCount . "...\n", $this->Ini->Nm_lang, true);
           ...
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['this_chart_label'] = '';
       ...
       if ($bChartFP)
       {
           ...
           grid_appointments_staff_pdf_progress_call(100 . "_#NM#_" . 90 . "_#NM#_" . $lang_protect . "...\n", $this->Ini->Nm_lang);
           fwrite($this->progress_fp, 90 . "_#NM#_" . $lang_protect . "...\n");
           fclose($this->progress_fp);
       }
       if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['charts_html']) && '' != $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['charts_html'])
       {
            $nm_saida->saida("<script type=\"text/javascript\">\r\n");
            $nm_saida->saida("{$_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['charts_html']}\r\n");
            $nm_saida->saida("</script>\r\n");
       }
   }
       $nm_saida->saida("</body>\r\n");
...
//--- 
 ...
 {
   global $nm_saida, $nm_lang;
   if (!empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['chart_list']))
   {
       foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['chart_list'] as $arr_chart)
       {
           $nm_saida->saida("<div style=\"page-break-after: always;\"><span style=\"display: none;\">&nbsp;</span></div>\r\n");
       $nm_saida->saida("<b><div style=\"height:1px;overflow:hidden\"><H1 style=\"font-size:0;padding:1px\">" . $this->Ini->Nm_lang['lang_btns_chrt_pdff_hint'] . "</H1></div></b>\r\n");
           $nm_saida->saida("<table><tr><td>\r\n");
...
//--- 
   ...
   {
      global $nm_saida;
      if ($this->Ini->Embutida_iframe || $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['embutida'] || $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['dashboard_info']['compact_mode'])
      { 
          return;
      } 
      ...
      if (!empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['cond_pesq']))
      {  
          $pos       = 0;
          $trab_pos  = false;
          $pos_tmp   = true; 
          $tmp       = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['cond_pesq'];
          while ($pos_tmp)
          {
             $pos = strpos($tmp, "##*@@", $pos);
             if ($pos !== false)
          ...
          $nm_cond_filtro_or  = (substr($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['cond_pesq'], $trab_pos + 5) == "or")  ? " " . trim($this->Ini->Nm_lang['lang_srch_orr_cond']) . " " : "";
          $nm_cond_filtro_and = (substr($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['cond_pesq'], $trab_pos + 5) == "and") ? " " . trim($this->Ini->Nm_lang['lang_srch_and_cond']) . " " : "";
          $nm_cab_filtro   = substr($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['cond_pesq'], 0, $trab_pos);
     ...
     if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['proc_pdf'] || $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['proc_pdf_vert']) {
      ...
      $nm_saida->saida("    <div class=\"" . $this->css_scGridHeaderFont . "\" style=\"float: left; text-transform: uppercase;\">Resumen de Horas De  Actividades - Bitácora Personal</div>\r\n");
   ...
   {
      //----- " . $this->Ini->Nm_lang['lang_othr_cons_title_YYYYMM'] . "
      if (!isset($this->array_total_appointment_start_date[$quebra_appointment_start_date_orig]))
      {
         $this->array_total_appointment_start_date[$quebra_appointment_start_date_orig][0] = 1;
         $this->array_total_appointment_start_date[$quebra_appointment_start_date_orig][1] = $sum_intervalo;
         $this->array_total_appointment_start_date[$quebra_appointment_start_date_orig][2] = $quebra_appointment_start_date;
         $this->array_total_appointment_start_date[$quebra_appointment_start_date_orig][3] = $quebra_appointment_start_date_orig;
      }
      else
      {
         $this->array_total_appointment_start_date[$quebra_appointment_start_date_orig][0]++;
         $this->array_total_appointment_start_date[$quebra_appointment_start_date_orig][1] = bcadd($this->array_total_appointment_start_date[$quebra_appointment_start_date_orig][1], $sum_intervalo, 10);
      }
      //----- current_status_id
      if (!isset($this->array_total_current_status_id[$quebra_appointment_start_date_orig][$quebra_current_status_id_orig]))
      {
         $this->array_total_current_status_id[$quebra_appointment_start_date_orig][$quebra_current_status_id_orig][0] = 1;
         $this->array_total_current_status_id[$quebra_appointment_start_date_orig][$quebra_current_status_id_orig][1] = $sum_intervalo;
         $this->array_total_current_status_id[$quebra_appointment_start_date_orig][$quebra_current_status_id_orig][2] = $quebra_current_status_id;
         $this->array_total_current_status_id[$quebra_appointment_start_date_orig][$quebra_current_status_id_orig][3] = $quebra_current_status_id_orig;
      }
      else
      {
         $this->array_total_current_status_id[$quebra_appointment_start_date_orig][$quebra_current_status_id_orig][0]++;
         $this->array_total_current_status_id[$quebra_appointment_start_date_orig][$quebra_current_status_id_orig][1] = bcadd($this->array_total_current_status_id[$quebra_appointment_start_date_orig][$quebra_current_status_id_orig][1], $sum_intervalo, 10);
      }
   }
...
$_SESSION['scriptcase']['grid_appointments_staff']['contr_erro'] = 'off'; 
     ...
     { 
         $nmgp_select = "SELECT appointment_start_date, appointment_start_time, appointment_end_time, intervalo, id_proyecto, id_actividad, nombre, observaciones, current_status_id, appointments_id from " . $this->Ini->nm_tabela; 
     } 
     else 
     { 
         $nmgp_select = "SELECT appointment_start_date, appointment_start_time, appointment_end_time, intervalo, id_proyecto, id_actividad, nombre, observaciones, current_status_id, appointments_id from " . $this->Ini->nm_tabela; 
     ...
     $campos_order = "";
     $format       = $this->Ini->Get_Gb_date_format('status', 'appointment_start_date');
     $campos_order = $this->Ini->Get_date_order_groupby("appointment_start_date", 'asc', $format, $campos_order);
     $format       = $this->Ini->Get_Gb_date_format('status', 'current_status_id');
     $campos_order = $this->Ini->Get_date_order_groupby("current_status_id", 'asc', $format, $campos_order);
...
// 
   function nm_acum_res_unit($rs_res, $nm_tipo="resumo")
   {
            ...
            $this->appointment_start_date = $rs_res->fields[0] ;  
            $this->appointment_start_time = $rs_res->fields[1] ;  
            $this->appointment_end_time = $rs_res->fields[2] ;  
            $this->intervalo = $rs_res->fields[3] ;  
            $this->id_proyecto = $rs_res->fields[4] ;  
            $this->id_actividad = $rs_res->fields[5] ;  
            $this->nombre = $rs_res->fields[6] ;  
            $this->observaciones = $rs_res->fields[7] ;  
            $this->current_status_id = $rs_res->fields[8] ;  
            $this->appointments_id = $rs_res->fields[9] ;  
...
{
    echo "<script>changeBackgroundText('#87B406','ENVIAR REGISTROS');</script>";
...
$_SESSION['scriptcase']['grid_appointments_staff']['contr_erro'] = 'off'; 
            ...
            $this->appointment_start_date_orig = $this->Ini->Get_arg_groupby($TP_Time . $this->appointment_start_date, $Format_tst);
            $this->appointment_start_date = $this->Ini->GB_date_format($TP_Time . $this->appointment_start_date, $Format_tst, $Prefix_dat, "N", "F/Y");
            $this->current_status_id_orig = $this->current_status_id;
            $this->current_status_id = $this->look_current_status_id ;
            if ($nm_tipo == "resumo")
            {
