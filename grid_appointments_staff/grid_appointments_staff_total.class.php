...
class grid_appointments_staff_total
{
   ...
   function Calc_resumo_status($destino_resumo, $res_export=false)
   {
      global $nm_lang;
      $this->nm_data = new nm_data("es");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['sql_tot_res']);
      ...
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('appointment_start_date' => "appointment_start_date",'current_status_id' => "current_status_id");
      $cmps_quebra_atual = array("appointment_start_date","current_status_id");
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['arr_total'][$cmp_gb] = $this->$Arr_tot_name;
      }
   }
   ...
   function Res_Totaliza_status($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def, $res_export)
   {
     ...
     $comando .= " ON " . $join;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['sql_tot_res']))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['sql_tot_res'] = str_replace("#@#cmps_quebras#@#", "", $comando);
      }
      $comando  = str_replace("#@#cmps_quebras#@#", "," . $cmps_quebras, $comando);
      $comando .= " group by " . $cmps_quebras . " order by " .  $cmps_quebras;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['Res_search_metric_use']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['Res_search_metric_use']))
      {
          $comando = $cmd_simp;
          $cmps_S  = "";
   ...
   {
      global $nada, $nm_lang , $id_proyecto, $id_actividad, $current_status_id;
      ...
      $nm_comando = "select count(*), sum(intervalo) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['where_pesq']; 
      ...
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $rt->fields[1] = (string)$rt->fields[1]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['tot_geral'][2] = $rt->fields[1]; 
      $rt->Close(); 
   ...
   {
      global $tot_appointment_start_date, $id_proyecto, $id_actividad, $current_status_id;
   ...
   //-----  current_status_id
   function quebra_current_status_id_status($appointment_start_date, $current_status_id, $arg_sum_appointment_start_date, $arg_sum_current_status_id) 
   {
      global $tot_current_status_id, $id_proyecto, $id_actividad, $current_status_id;
      $tot_current_status_id = array() ;  
      $nm_comando = "select count(*), sum(intervalo) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['where_pesq']; 
      if (empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_staff']['where_pesq'])) 
      { 
         $nm_comando .= " where " .  $this->Sc_groupby_appointment_start_date . $arg_sum_appointment_start_date . " and current_status_id" . $arg_sum_current_status_id ; 
      } 
      else 
      { 
         $nm_comando .= " and " .  $this->Sc_groupby_appointment_start_date . $arg_sum_appointment_start_date . " and current_status_id" . $arg_sum_current_status_id ; 
      } 
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando;
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = '';
      if (!$rt = $this->Db->Execute($nm_comando)) 
