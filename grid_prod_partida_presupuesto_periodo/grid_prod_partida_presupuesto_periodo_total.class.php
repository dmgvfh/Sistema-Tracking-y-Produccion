...
class grid_prod_partida_presupuesto_periodo_total
{
   var $Db;
   var $Erro;
   var $Ini;
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['grid_prod_partida_presupuesto_periodo']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['grid_prod_partida_presupuesto_periodo']['campos_busca']))
      { 
   ...
   //----- 
   function Calc_resumo_agrupa_item($destino_resumo, $res_export=false)
   {
      global $nm_lang, $no_facturado, $tipo_moneda, $valor_unitario, $utilizado, $id_partida, $total_valorizado;
      $this->nm_data = new nm_data("es");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_prod_partida_presupuesto_periodo']['sql_tot_res']);
      ...
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_prod_partida_presupuesto_periodo']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('prod_agrupa_items_nombre_agrupador' => "nombre_agrupador");
      $cmps_quebra_atual = array("prod_agrupa_items_nombre_agrupador");
      ...
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          ...
          $this->Res_Totaliza_agrupa_item($ind_qb, $cmp_gb, $arr_tots, $group, $join, $cmps_gb, $cmps_gb1, $cmps_gb2, $cmps_gbS, $cmp_dim, $cmps_quebra_atual, $cmp_sql_def, $res_export);
      }
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_prod_partida_presupuesto_periodo']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_prod_partida_presupuesto_periodo']['arr_total'][$cmp_gb] = $this->$Arr_tot_name;
      }
   }

   function Res_Totaliza_agrupa_item($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def, $res_export)
   {
      $sc_having = ((isset($parms_sub_sel['having']))) ? "  having " . $parms_sub_sel['having'] : "";
      $Tem_estat_manual = false;
      $where_ok = $this->sc_where_atual;
      $cmp_sql_tp_num = array('prod_partida_presupuesto_periodo_id' => 'N','prod_partida_presupuesto_periodo_id_presupuesto_periodo' => 'N','prod_partida_presupuesto_periodo_id_proyecto' => 'N','prod_partida_presupuesto_periodo_monto_uf' => 'N','prod_partida_presupuesto_periodo_avance' => 'N','prod_partida_presupuesto_periodo_unidades_periodo' => 'N','prod_partida_presupuesto_periodo_id_item' => 'N','prod_partida_presupuesto_periodo_id_periodo_factura' => 'N','tipo_moneda' => 'N','total_valorizado' => 'N','valor_unitario' => 'N');
      $nm_comando = "select distinct   prod_partida_presupuesto_periodo.id,   prod_partida_presupuesto_periodo.id_presupuesto_periodo,   prod_partida_presupuesto_periodo.id_proyecto,   prod_items_base.item,   prod_partida_presupuesto_periodo.monto_uf,   prod_partida_presupuesto_periodo.avance,   prod_agrupa_items.nombre_agrupador,   prod_partida_presupuesto_periodo.unidades_periodo,   prod_partida_presupuesto_periodo.id_item, prod_partida_presupuesto_periodo.observaciones, prod_partida_presupuesto_periodo.id_periodo_factura from " . $this->Ini->nm_tabela . " " . $this->sc_where_atual; 
     $cmd_simp = "select count(*), sum(avance)#@#cmps_quebras#@# from (" . $nm_comando . ") sc_sel_esp " . $where_ok;
     $comando  = "select count(*), sum(SC_metric1)#@#cmps_quebras#@# from (";
     $comando .= "select avance as SC_metric1, " . $cmps_quebras1 . " from (" . $nm_comando . ") sc_sel_esp1 " .  $where_ok . ") SC_sel1 INNER JOIN (";
     $comando .= "select " . $cmps_quebras2 . " from (" . $nm_comando . ") sc_sel_esp2 " .  $where_ok . " group by " . $group . $sc_having . ") SC_sel2 ";
     $comando .= " ON " . $join;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_prod_partida_presupuesto_periodo']['sql_tot_res']))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_prod_partida_presupuesto_periodo']['sql_tot_res'] = str_replace("#@#cmps_quebras#@#", "", $comando);
      }
      $comando  = str_replace("#@#cmps_quebras#@#", "," . $cmps_quebras, $comando);
      $comando .= " group by " . $cmps_quebras . " order by " .  $cmps_quebras;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_prod_partida_presupuesto_periodo']['Res_search_metric_use']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_prod_partida_presupuesto_periodo']['Res_search_metric_use']))
      {
          $comando = $cmd_simp;
          $cmps_S  = "";
      ...
      $format_dimensions['prod_agrupa_items_nombre_agrupador']['reg'] = "S";
      $format_dimensions['prod_agrupa_items_nombre_agrupador']['msk'] = "";
      while (!$rt->EOF)
      {
          $sql_where = "";
          foreach ($Cmp_dim as $Cada_dim => $Ind_sql)
          {
              ...
              $SC_prep = $this->Ini->Get_format_dimension($Ind_sql, 'agrupa_item', $Cada_dim, $rt, $format_dimensions[$Cada_dim]['reg'], $format_dimensions[$Cada_dim]['msk']);
              $SC_orig = $Cada_dim . "_orig";
              $SC_graf = "val_grafico_" . $Cada_dim;
              $$Cada_dim = $SC_prep['fmt'];
              $$SC_orig = $SC_prep['orig'];
              if ($Cada_dim == "prod_agrupa_items_nombre_agrupador") {
              }
              if (null === $$Cada_dim)
              {
                  $$Cada_dim = '';
          ...
          if ($Tem_estat_manual)
          {
              ...
              $vl_statistic = $this->Calc_statist_manual_agrupa_item($where_ok);
   ...
   //---- 
   function quebra_geral_agrupa_item($res_limit=false, $res_export=false)
   {
      global $nada, $nm_lang ;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_prod_partida_presupuesto_periodo']['contr_total_geral'] == "OK") 
      { 
          return; 
      } 
      $nm_comando = "select distinct   prod_partida_presupuesto_periodo.id,   prod_partida_presupuesto_periodo.id_presupuesto_periodo,   prod_partida_presupuesto_periodo.id_proyecto,   prod_items_base.item,   prod_partida_presupuesto_periodo.monto_uf,   prod_partida_presupuesto_periodo.avance,   prod_agrupa_items.nombre_agrupador,   prod_partida_presupuesto_periodo.unidades_periodo,   prod_partida_presupuesto_periodo.id_item, prod_partida_presupuesto_periodo.observaciones, prod_partida_presupuesto_periodo.id_periodo_factura from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_prod_partida_presupuesto_periodo']['where_pesq']; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_prod_partida_presupuesto_periodo']['tot_geral'] = array() ;  
      $nm_comando = "select count(*), sum(avance) from (" . $nm_comando . ") nm_sel_esp "; 
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_prod_partida_presupuesto_periodo']['tot_geral'][0] = "" . $this->Ini->Nm_lang['lang_msgs_totl'] . ""; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_prod_partida_presupuesto_periodo']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $rt->fields[1] = (string)$rt->fields[1]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_prod_partida_presupuesto_periodo']['tot_geral'][2] = $rt->fields[1]; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_prod_partida_presupuesto_periodo']['contr_total_geral'] = "OK";
   } 

   //-----  prod_agrupa_items_nombre_agrupador
   function quebra_prod_agrupa_items_nombre_agrupador_agrupa_item($prod_agrupa_items_nombre_agrupador, $arg_sum_prod_agrupa_items_nombre_agrupador) 
   {
      ...
      { 
         $nm_comando .= " where prod_agrupa_items.nombre_agrupador" . $arg_sum_prod_agrupa_items_nombre_agrupador ; 
      } 
      else 
      { 
         $nm_comando .= " and prod_agrupa_items.nombre_agrupador" . $arg_sum_prod_agrupa_items_nombre_agrupador ; 
      } 
      $tot_prod_agrupa_items_nombre_agrupador = array() ;  
      $nm_comando = "select count(*), sum(avance) from (" . $nm_comando . ") nm_sel_esp "; 
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando;
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = '';
      if (!$rt = $this->Db->Execute($nm_comando)) 
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_prod_partida_presupuesto_periodo']['opcao'] == "pdf" && isset($_SESSION['nm_session']['sys_wkhtmltopdf_show_html_content']) && $_SESSION['nm_session']['sys_wkhtmltopdf_show_html_content'] == 'Y') {
          $tot_prod_agrupa_items_nombre_agrupador[0] = NM_encode_input(sc_strip_script($prod_agrupa_items_nombre_agrupador));
      }
      else {
          $tot_prod_agrupa_items_nombre_agrupador[0] = sc_strip_script($prod_agrupa_items_nombre_agrupador) ; 
      }
      $tot_prod_agrupa_items_nombre_agrupador[1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $tot_prod_agrupa_items_nombre_agrupador[2] = (string)$rt->fields[1]; 
      $rt->Close(); 
   } 
